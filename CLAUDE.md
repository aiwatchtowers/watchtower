...
- Discuss chat (`SituationDiscussSection` / `SituationChatViewModel`, `chat_conversations.context_type='situation'`) — collapsed per-situation secretary chat with a Draft-reply contract (owner's voice via `workspace.style_profile` + counterparty people-card briefs + a register sample of the owner's own recent messages); style profile generated by `watchtower inbox style-sample` (`inbox.style_sample`, strong tier), edited in the Profile tab (`SecretaryProfileViewModel` in `AppState`).
