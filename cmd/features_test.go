...
// disableFeatureInConfig appends `<block>: {enabled: false}` to the test
// config so an enable test starts from OFF — the feature under test may be on
// by default (inbox, reaction-commands), and `features enable` on an
// already-enabled feature is a guarded no-op.
func disableFeatureInConfig(t *testing.T, path, block string) {
	t.Helper()
...
func TestFeaturesEnable_RunsFastForward(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	disableFeatureInConfig(t, writeLegacyConfig(t, ""), "inbox")
...
func TestFeaturesEnable_ReactionCommandsReportsSeededCount(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	// The feature is on by default (2026-09-26); this test is about enabling
	// it from OFF, so the config must say so explicitly.
	disableFeatureInConfig(t, writeLegacyConfig(t, ""), "reaction_commands")
...
// TestFeaturesEnable_AlreadyEnabledSkipsFastForward pins the guard the
// default-on flip makes load-bearing: `features enable` on a feature that is
// already on must NOT run its fast-forward hook. For reaction-commands the
// hook re-seeds the ledger against an account the daemon is actively polling,
// recording every not-yet-dispatched reaction (cap-deferred, transient-retry,
// just placed) as permanent `skipped`; for the watermark features it would
// silently skip whatever the daemon had not processed yet. The Desktop's
// `enableNow` already guards this; the CLI did not.
func TestFeaturesEnable_AlreadyEnabledSkipsFastForward(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeLegacyConfig(t, "") // no reaction_commands block = on by default
	...
	assert.Equal(t, 0, seeds, "an already-enabled feature must not be re-seeded")
...
func TestFeaturesEnable_FailedFastForwardLeavesKeyUnwritten(t *testing.T) {
	...
	disableFeatureInConfig(t, configPath, "inbox")
