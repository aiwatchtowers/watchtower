...
func runIdeasMine(cmd *cobra.Command, _ []string) error {
	...
	// lock-release-and-floor-restore (internal/ideas/backfill.go's Backfill,
	// `defer p.restoreBackfillFloors(...)`) instead of the process dying
	// with the lock held or a lowered floor left behind (GB8, IDEA-01 floor
	// honesty). notifyShutdownContext's second-signal-kills behavior would
	// terminate the process outright — including mid-defer, if the second
	// signal lands while the restore is still running — so the backfill
	// path deliberately stays on plain signal.NotifyContext: the first
	// signal cancels ctx exactly as before, letting the defer run to
	// completion; a second signal is swallowed (the pre-notifyShutdownContext
	// shape) rather than printing a hint that would invite an operator to
	// send the very signal that could cut the restore short.
	if fromStr != "" {
	...
	// The incremental path has no floor-restore defer to protect (each AI
	// call commits its own floor advance as it completes), so it keeps the
	// standard second-signal-kills helper.
	ctx, cancel := notifyShutdownContext(ctx, logger.Printf)
	defer cancel()
