...
// this logger. The stdlib default logger is pointed at this stream for the
// daemon's lifetime (routeStdLog) and the Jira sub-components get sub-loggers
// on it (wireJiraSyncers), so what remains on stderr is only code that writes
// to os.Stderr directly. Passing the flag keeps that decision visible at the
...
// subLogger returns a logger writing to parent's destination under its own
// prefix, so a component wired onto the daemon's stream keeps the tag
// ("[jira-analyzer] ", …) its stderr default carried and stays greppable.
func subLogger(parent *log.Logger, prefix string) *log.Logger {
	return log.New(parent.Writer(), prefix, parent.Flags())
}

// routeStdLog points the standard library's default logger at logger's
// destination and returns a func restoring the previous one. The daemon calls
// it once at start: packages reached from daemon phases (internal/db,
// internal/ai, internal/jira's dashboards, …) log through the package-level
// log.Printf, whose default os.Stderr is daemon.log for a detached child — the
// one file that cannot be rotated while the daemon runs. Writing to
// logger.Writer() (not through logger itself) keeps each line single: under
// --verbose that writer already tees to stderr exactly once. Only `sync
// --daemon` calls it: other commands, the one-shot sync included, keep their
// package-level log.Printf output on the terminal. The restore is deferred
// after logFile.Close in runSync, so it runs first and nothing writes to the
// closed file through the default logger.
func routeStdLog(logger *log.Logger) (restore func()) {
	prev := log.Writer()
