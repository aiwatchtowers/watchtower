...
func TestJiraFeaturesEnable_AcceptsLongName(t *testing.T) {
	...
	require.NoError(t, setJiraFeatureToggle(newJiraFeaturesTestCmd(), "without_jira_detection", true))

	features := loadJiraFeatures(t, configPath)
	assert.True(t, features.EpicProgress)
	assert.True(t, features.WithoutJiraDetection, "the long spelling must write the same key as the short one")
}
...
func TestJiraFeatureConfigKeys_MatchStructAndToggleRef(t *testing.T) {
	...
	// and is the one table nothing else checks: a new toggle added to the
	// struct and to jiraFeatureConfigKeys but forgotten here would be left
	// out of reset and omit a row, silently.
