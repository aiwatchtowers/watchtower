...
func connectJiraAccount(cmd *cobra.Command, cfg *config.Config, database *db.DB, accountID int64, token *jira.OAuthToken, siteFlag, preferCloudID string, isNewRow bool) (jira.CloudResource, error) {
	...
	// is retried on the next daemon start (wireJiraSyncers' lazy fill).
	client := jira.NewClient(site.ID, resolveJiraOAuthConfig(), store)
	recordJiraOwner(cmd.Context(), cmd.ErrOrStderr(), database, accountID, client)
...
var featureNames = []string{
	"my_issues", "awaiting_input", "track_linking",
	"team_workload", "blocker_map", "iteration_progress", "epic_progress",
	"release_dashboard", "without_jira",
}
...
func runJiraFeatures(cmd *cobra.Command, _ []string) error {
	...
	if err == nil {
		defer database.Close()
		role = ownerJiraFeaturesRole(database, role)
	}
...
// false forever. Writing one scalar key also leaves every other toggle alone,
// instead of cementing explicit falses read out of an all-false struct.
...
func runJiraFeaturesReset(cmd *cobra.Command, _ []string) error {
	...
	// One scalar key per toggle in a single write, for the reason setJiraFeatureToggle
...
// ownerJiraFeaturesRole is the owner profile's role when one is set, else
// fallback. No owner or no profile is not an error here: picking Jira feature
// defaults does not need to know who the owner is.
func ownerJiraFeaturesRole(database *db.DB, fallback string) string {
	owner, err := database.ResolveOwner()
	if err != nil {
		return fallback
	}
	if profile, _ := database.GetOwnerProfile(owner); profile != nil && profile.Role != "" {
		return profile.Role
	}
	return fallback
}
