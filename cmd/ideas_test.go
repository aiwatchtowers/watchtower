...
// TestIdeasBackfillSignalHelperProcess is not a real test: it is re-executed
// as a subprocess (the GO_WANT_HELPER_PROCESS pattern, see
// cmd/shutdown_test.go) to prove the exact signal primitive runIdeasMine's
// --from branch is wired to — plain signal.NotifyContext, not
// notifyShutdownContext — never kills the process on a second SIGTERM.
// That's the property IDEA-01 depends on: internal/ideas/backfill.go's
// Backfill defers its floor restore, and only a killed process (not an
// ordinary cancelled-ctx return) can skip a defer. Run the ordinary way it
// is a silent no-op.
func TestIdeasBackfillSignalHelperProcess(t *testing.T) {
	...
	// The real Backfill keeps running its deferred restore after ctx is
	// cancelled; this blocks forever so the test can prove a second SIGTERM
	// does not cut that short — only SIGKILL (sent at cleanup) may end it.
	select {}
}

// TestIdeasBackfillSignalDisposition_SecondSignalDoesNotKill pins the fix
// for the ideas-mine-backfill/notifyShutdownContext regression (a second
// signal must never kill the backfill path outright, unlike every other
// notifyShutdownContext call site) by exercising the actual primitive
// runIdeasMine wires up for --from, not a stand-in.
func TestIdeasBackfillSignalDisposition_SecondSignalDoesNotKill(t *testing.T) {
	helper := exec.Command(os.Args[0], "-test.run=^TestIdeasBackfillSignalHelperProcess$")
	...
	waitForLine := func(want string, deadline time.Duration) {
		t.Helper()
		select {
		case line, ok := <-lines:
			if !ok || line != want {
				t.Fatalf("helper did not print %q (got %q, ok=%v)", want, line, ok)
			}
	...
	waitForLine("ready", 5*time.Second)
	require.NoError(t, helper.Process.Signal(syscall.SIGTERM))
	waitForLine("cancelled", 2*time.Second)
	...
	// Send a burst of second signals over a real window; if the process
	// dies from any of them, this is the notifyShutdownContext shape —
	// exactly what the backfill path must NOT have.
...
loop:
	for {
		...
		case werr := <-waitCh:
			t.Fatalf("helper exited on a second SIGTERM (err=%v) — the backfill path must swallow it, not kill the process, or IDEA-01's floor-restore defer could be cut short", werr)
		}
	}
