...
final class ChannelStatsTests: XCTestCase {
    ...
    /// Channel stats match `messages.user_id`, so they genuinely need Slack:
    /// a Google-only owner (id `google:…`) yields no Slack id, never the owner key.
    func testOwner01FetchCurrentUserIDGoogleOnlyIsNil() throws {
        let db = try TestDatabase.create()
        try db.write { db in
