...
final class SlackTextParserTests: XCTestCase {
    ...
    func testUserMentionResolvesViaLookup() {
        let input = "Ping <@U0FAKE02> about this"
        let result = SlackTextParser.toPlainText(input, userNames: ["U0FAKE02": "Jane Doe"])
        XCTAssertEqual(result, "Ping @Jane Doe about this")
    }

    func testUserMentionFallsBackToIDWhenUnknown() {
        let input = "Ping <@U0FAKE02> about this"
        XCTAssertEqual(SlackTextParser.toPlainText(input), "Ping @U0FAKE02 about this")
    }

    func testUserMentionWithInlineNameIgnoresLookup() {
        let input = "Hey <@U0FAKE02|jane>"
        let result = SlackTextParser.toPlainText(input, userNames: ["U0FAKE02": "Jane Doe"])
        XCTAssertEqual(result, "Hey @jane")
    }
    ...
    func testAttributedStringAutolinksBareURLs() {
        let input = "See https://acme-dev.atlassian.net/browse/REG-4 now"
        let result = SlackTextParser.toAttributedString(input)
        var foundLink = false
        for run in result.runs where run.link != nil {
            foundLink = true
            XCTAssertEqual(run.link?.absoluteString, "https://acme-dev.atlassian.net/browse/REG-4")
        }
        XCTAssertTrue(foundLink, "Bare URL should become a clickable link")
    }

    func testAttributedStringResolvesUserMentionViaLookup() {
        let input = "Hey <@U0FAKE02>"
        let result = SlackTextParser.toAttributedString(input, userNames: ["U0FAKE02": "Jane Doe"])
        XCTAssertEqual(String(result.characters), "Hey @Jane Doe")
    }
