...
final class ActionStripViewModelTests: XCTestCase {
    ...
    /// `reaction-commands` run (`trackedPipelineRun` in the daemon) — a
    /// running or failed poll, or another pipeline's run, is not a check.
    func testRefreshReadsTheLatestFinishedReactionCommandsRun() async throws {
        let (pool, path) = try makePool()
        defer { TestDatabase.cleanup(path: path) }
        try await pool.write { db in
            try db.execute(sql: """
                INSERT INTO pipeline_runs (pipeline, source, status, started_at, finished_at) VALUES
                ('reaction-commands','daemon','done','2026-09-25T08:00:00Z','2026-09-25T08:00:05Z'),
                ('reaction-commands','daemon','done','2026-09-25T09:00:00Z','2026-09-25T09:00:04Z'),
                ('reaction-commands','daemon','error','2026-09-25T10:00:00Z','2026-09-25T10:00:02Z'),
                ('reaction-commands','daemon','running','2026-09-25T11:00:00Z',NULL),
                ('slack-sync','daemon','done','2026-09-25T12:00:00Z','2026-09-25T12:00:09Z')
                """)
        }
        let vm = ActionStripViewModel(dbPool: pool, cliRunner: FakeCLIRunner())
        ...
        XCTAssertEqual(vm.lastReactionCheck, "2026-09-25T09:00:04Z")
        XCTAssertNil(vm.lastError)
    }

    func testRefreshLeavesLastReactionCheckNilWithoutARun() async throws {
        let (pool, path) = try makePool()
        defer { TestDatabase.cleanup(path: path) }
        let vm = ActionStripViewModel(dbPool: pool, cliRunner: FakeCLIRunner())
        ...
        XCTAssertNil(vm.lastReactionCheck)
    }
}
