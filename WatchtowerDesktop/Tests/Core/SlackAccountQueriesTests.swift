...
final class SlackAccountQueriesTests: XCTestCase {
    ...
    /// Link resolution needs every account's team, including disabled and
    /// removed ones — their already-synced data still renders links.
    func testFetchTeamIDsIncludesDisabledAndRemovedAccounts() throws {
        let pool = try makePool()
        let (active, disabled, removed) = try pool.write { db -> (Int64, Int64, Int64) in
            (try TestDatabase.insertSlackAccount(db, teamID: "T001"),
             try TestDatabase.insertSlackAccount(db, teamID: "T002", enabled: false),
             try TestDatabase.insertSlackAccount(db, teamID: "T003", status: "removed", enabled: false))
        }

        let teams = try pool.read { db in try SlackAccountQueries.fetchTeamIDs(db) }

        XCTAssertEqual(teams, [Int(active): "T001", Int(disabled): "T002", Int(removed): "T003"])
    }
