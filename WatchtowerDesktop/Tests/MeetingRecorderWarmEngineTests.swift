...
final class MeetingRecorderWarmEngineTests: MeetingRecorderTestCase {
    ...
    func testTickInTheHandoverGapDoesNotPrewarmASecondEngine() async throws {
        ...
        // The tick runs from the Center's handover seam — the very main-actor
        // step that moves the engine onto the job — so it lands in the gap by
        // construction rather than by out-racing the job's task (a yield loop
        // polling for the gap flaked on a loaded CI runner). The gap state is
        // still asserted, so the test cannot pass by ticking somewhere harmless.
        var sawGap = false
        var prewarmedInGap = false
        var handovers = 0
        center.onLiveEngineHandedToJobForTesting = { [weak center] in
            guard let center else { return XCTFail("the handover seam fired after the center was deallocated") }
            handovers += 1
            sawGap = center.jobs.first?.engine != nil && !center.isCapturing && center.activeJobID == nil
            center.warmPolicyTick(config: config)
            prewarmedInGap = center.isPrewarming
        }
        await center.stopAndProcess(config: config)

        XCTAssertEqual(handovers, 1, "the live engine must be handed to the job exactly once")
        XCTAssertTrue(sawGap, "the handover gap was never reached — the tick never exercised it")
        XCTAssertFalse(prewarmedInGap, "a job holding the handed-over engine counts as busy")
