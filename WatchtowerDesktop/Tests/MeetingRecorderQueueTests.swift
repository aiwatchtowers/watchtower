...
final class MeetingRecorderQueueTests: MeetingRecorderTestCase {
    ...
    func testRecordingStartsWhileJobTranscribesAndLivePassWaitsForTheSlot() async throws {
        ...
        let loaded = await waitUntil("the waiting live pass to load its engine") {
            engineLoads >= 2 && center.liveEngineState != .waiting
        }
    ...
    func testSecondStopQueuesBehindTheRunningJobAndBothSave() async throws {
        ...
        // How many transcripts had been saved when each job claimed the engine
        // slot: a job that started alongside job 1 would claim at 0, not 1.
        var savesAtClaim: [Int] = []
        center.onEngineSlotClaimedForTesting = { savesAtClaim.append(runner.savedTranscripts.count) }
        let config = singleWindowConfig()
        ...
        let queued = await waitUntil("the second stop to enqueue its job") { center.jobs.count == 2 }
        guard queued else { gate.release(); return }
        ...
        XCTAssertEqual(savesAtClaim, [0, 1],
                       "the second job claims the engine only after the first one saved")
    ...
    func testRecordingStoppedWhileWaitingNeverConsumesThePriorLiveTail() async throws {
        ...
        // Recording 1's stop ends capture and enqueues its job in one step,
        // before it parks on the draining tail.
        let firstQueued = await waitUntil("the first stop to enqueue its job") { center.jobs.count == 1 }
        guard firstQueued else { gate.release(); return }
        ...
        let stopSecond = Task { await center.stopAndProcess(config: config) }
        let secondQueued = await waitUntil("the second stop to enqueue its job") { center.jobs.count == 2 }
        guard secondQueued else { gate.release(); return }
    ...
    func testStopErrorHandsTheEngineSlotOnOnlyAfterTheOrphanExits() async throws {
        ...
        // Waiting cannot prove job A stayed parked, so the claim seam records
        // whether it took the slot while the orphan was still inside the engine.
        var claimedWhileOrphanResident = false
        center.onEngineSlotClaimedForTesting = {
            if liveGate2.completedCalls == 0 { claimedWhileOrphanResident = true }
        }
        recorder2.stopError = AudioRecordingError.deviceSetupFailed("device vanished")
        await center.stopAndProcess(config: config)
        // A wake-up the failed stop triggered is already enqueued: let it run
        // (and claim, if it wrongly can) before the orphan is released.
        ...
        guard ran else { return }
        XCTAssertFalse(claimedWhileOrphanResident,
                       "job A must not claim the engine slot while the orphaned live pass is resident")
        await stopA.value
