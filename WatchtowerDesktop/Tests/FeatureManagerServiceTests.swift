...
// MARK: - enableNow() — the Inbox Strip's one-click enable

extension FeatureManagerServiceTests {
    @Test("enableNow() runs only `features enable <id>`, restarts once and reloads — never the rest of the staged batch")
    func enableNowLeavesOtherStagedChangesAlone() async {
        let (service, runner) = Self.makeService(stdout: Self.featuresListJSON)
        await service.load()
        service.setPending("ideas", enabled: false)
        let spy = RestartSpy()

        await service.enableNow("tracks") { await spy.restart() }
        ...
        #expect(spy.callCount == 1)
        #expect(service.pending == ["ideas": false], "a decision staged in Settings is not this button's to apply")
        #expect(service.loadError == nil)
        #expect(service.isApplying == false)
    }

    @Test("enableNow() drops a staged entry for the same id once the enable lands")
    func enableNowDropsItsOwnStagedEntry() async {
        let (service, _) = Self.makeService(stdout: Self.featuresListJSON)
        await service.load()
        service.setPending("tracks", enabled: true)

        await service.enableNow("tracks") {}

        #expect(service.pending.isEmpty)
    }

    @Test("enableNow() surfaces a failed enable via loadError and skips the restart")
    func enableNowSurfacesFailure() async {
        let runner = NthCallFailingRunner(
            failOnCall: 3,
            error: CLIRunnerError.nonZeroExit(code: 1, stderr: "boom: slack unreachable"),
            stdout: Data(Self.featuresListJSON.utf8)
        )
        let service = FeatureManagerService(runner: runner)
        await service.load()
        ...
        #expect(spy.callCount == 0, "nothing went live, so there is nothing for the daemon to pick up")
        #expect(service.loadError?.contains("boom: slack unreachable") == true)
        #expect(service.isApplying == false)
    }

    @Test("enableNow() never re-runs `features enable` for a feature that is already on — the fast-forward hook would re-seed the reaction ledger")
    func enableNowSkipsAlreadyEnabledFeature() async {
        let (service, runner) = Self.makeService(stdout: Self.featuresListJSON)
        let spy = RestartSpy()

        // "ideas" is enabled in the live list, as after an out-of-band CLI enable.
        await service.enableNow("ideas") { await spy.restart() }
    ...
    @Test("enableNow() does not enable when the live state cannot be read")
    func enableNowSkipsWhenReloadFails() async {
        let runner = NthCallFailingRunner(
            failOnCall: 1,
    ...
    @Test("enableNow() surfaces a restart failure after a successful enable")
    func enableNowSurfacesRestartFailure() async {
        let (service, _) = Self.makeService(stdout: Self.featuresListJSON)
        await service.load()
        let spy = RestartSpy()

        await service.enableNow("tracks") {
            try await spy.throwingRestart(CLIRunnerError.nonZeroExit(code: 1, stderr: "restart failed"))
        }
