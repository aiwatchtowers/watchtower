...
final class GateEngine: WhisperWindowEngine, @unchecked Sendable {
    ...
    /// How many `transcribeWindow` calls have returned — a live pass parked in
    /// this engine is resident at least until this moves.
...
class MeetingRecorderTestCase: XCTestCase {
    ...
    /// Runs every main-actor job already enqueued before returning: the main
    /// executor is FIFO, so a task enqueued now runs behind all of them. Exact
    /// for work that is already scheduled — what a negative assertion needs to
    /// rule out a wake-up the step under test triggered synchronously — where
    /// a fixed yield count is only a guess.
    func runEnqueuedMainActorWork() async {
