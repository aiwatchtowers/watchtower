...
final class DayPlanViewModelTests: XCTestCase {
    ...
    /// OWNER-02: with no owner the CLI refuses `day-plan generate` with a
    /// non-zero exit — the reason must reach the banner, never a silent no-op.
    func testOwner02RegenerateSurfacesNoOwnerError() async throws {
        cli.shouldThrow = CLIRunnerError.nonZeroExit(
        ...
        XCTAssertFalse(vm.isGenerating)
        XCTAssertNil(vm.plan)
        let message = try XCTUnwrap(vm.generationError)
