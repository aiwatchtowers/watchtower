...
final class MeetingRecorderCenterTests: MeetingRecorderTestCase {
    ...
    func testRetryAfterStopErrorLoadsFreshEngineNotStale() async throws {
        ...
        await center.startRecording(eventID: nil, title: "Ad hoc")
        // The live pass must finish loading its engine (into `loadedEngine`,
        // set just before `.running`) before the recording is stopped.
        let running = await waitUntil("the live pass to load its engine") { center.liveEngineState == .running }
    ...
    func testProgressReported() async throws {
        ...
        // reported: window 1 → initial 0/0, window 2 → 1/3, window 3 → 2/3. The
        // gated engine produces no new progress while blocked, so `phase` settles
        // on exactly that value once the stream-backed consumer catches up.
        ...
        for phase in expected {
            ...
            await waitUntil("progress \(phase)") { center.phase == phase }
    ...
    func testLiveDisabledSkipsLivePassAndBatchTranscribesAfterStop() async throws {
        ...
        // No drain: a disabled start settles `.off` synchronously and spawns no
        // live task that could consume these samples. The checks below are a
        // synchronous snapshot — a live pass started anyway shows as `.loading`
        // here; its async work is caught after Stop by `engineLoads == 1` (a
        // second load) or `decodeCalls == 1` (a live result saved in place of
        // the batch decode), depending on whether the job adopts its engine.
    ...
    func testLiveDisabledRecordingOverDrainingOrphanTailDoesNotAdoptIt() async throws {
        ...
        // Recording 2's job is enqueued in the same step Stop decides it owns no
        // live pass — so the orphan was still parked when that decision fell.
        let queued = await waitUntil("recording 2's stop to enqueue its job") { center.jobs.count == 2 }
        guard queued else { gateEngine.release(); return }
        gateEngine.release()
        ...
        // The orphan's chunk landed before it exited — which recording 2's job
        // waited for — and no later live start clears the list, so this pins
        // the generation fence itself (a live-ON successor would mask it).
        XCTAssertTrue(center.liveChunks.isEmpty,
                      "the orphan's stale chunk must be fenced out of the live-disabled recording's list")
    ...
    func testLiveChunksAccumulateAndSurviveViewLifetime() async throws {
        ...
        await waitUntil("the live task to emit a chunk") { !center.liveChunks.isEmpty }
    ...
    func testLiveEngineUnavailableFallsBackToBatch() async throws {
        ...
        // The engine loads on a background task, so the .unavailable
        // transition lands after startRecording returns.
        await waitUntil("the live pass to give up") { center.liveEngineState == .unavailable }
    ...
    func testStopErrorCancelsOrphanedLiveTaskAndFencesStaleAppends() async throws {
        ...
        XCTAssertEqual(center.liveEngineState, .waiting,
                       "the orphan still holds the engine slot, so recording 2's live pass parks")

        // Now let the orphaned generation-1 engine call resume: its onChunk
        // fires, but is fenced by the (already-bumped) generation check. The
        // orphan's exit is what starts recording 2's parked live pass, and the
        // append it spawned was enqueued on the main actor before that exit —
        // so once the parked pass has started, the stale append has run. That
        // start also resets `liveChunks`, so this pins the outcome, not the
        // fence alone; the live-disabled orphan test pins the fence.
        gateEngine.release()
        let orphanExited = await waitUntil("the orphan to exit and hand the slot on") {
            center.liveEngineState != .waiting
        }
        guard orphanExited else { return }
