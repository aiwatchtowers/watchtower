        ...
        try db.execute(sql: """
            INSERT INTO agent_actions
                (tool, external, args_json, reason, surface, conversation_id, context_type, context_id,
                 turn_id, status, result_json, error, created_at, decided_at, applied_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, arguments: [tool, external, argsJSON, reason, surface, conversationID, contextType, contextID,
                             turnID, status, resultJSON, error, createdAt, decidedAt, appliedAt])
        return db.lastInsertedRowID
    }
