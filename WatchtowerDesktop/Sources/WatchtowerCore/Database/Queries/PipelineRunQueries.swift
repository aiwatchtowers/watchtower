...
package enum PipelineRunQueries {
    package static func fetchRecent(_ db: Database, limit: Int = 50) throws -> [PipelineRun] {
    ...
    /// `finished_at` of `pipeline`'s latest `done` run, or nil if it never
    /// finished one.
    package static func lastSuccessfulFinish(_ db: Database, pipeline: String) throws -> String? {
        try String.fetchOne(
            db,
            sql: """
                SELECT finished_at FROM pipeline_runs
                WHERE pipeline = ? AND status = 'done' AND finished_at IS NOT NULL
    ...
    package static func fetchSteps(_ db: Database, runId: Int64) throws -> [PipelineStepRecord] {
