...
package enum PromptQueries {

    package static func fetchAll(_ db: Database) throws -> [PromptTemplate] {
        guard try db.tableExists("prompts") else { return [] }
        return try PromptTemplate.fetchAll(db, sql: "SELECT * FROM prompts ORDER BY id")
    }

    package static func fetchByID(_ db: Database, id: String) throws -> PromptTemplate? {
    ...
    package static func fetchHistory(_ db: Database, promptID: String) throws -> [PromptHistoryEntry] {
