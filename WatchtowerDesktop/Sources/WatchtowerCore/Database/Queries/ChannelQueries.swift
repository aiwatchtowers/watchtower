...
package enum ChannelQueries {
    ...
    package static func fetchAll(_ db: Database, type: String? = nil) throws -> [Channel] {
    ...
    package static func fetchByID(_ db: Database, id: String) throws -> Channel? {
        try Channel.fetchOne(db, sql: "SELECT * FROM channels WHERE id = ?", arguments: [id])
    }

    package static func fetchByName(_ db: Database, name: String) throws -> Channel? {
        try Channel.fetchOne(db, sql: "SELECT * FROM channels WHERE name = ?", arguments: [name])
    }

    package static func fetchWatched(_ db: Database) throws -> [Channel] {
