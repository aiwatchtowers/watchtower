...
package enum PeopleCardQueries {
    ...
    package static func fetchLatestWindow(_ db: Database) throws -> (from: Double, to: Double)? {
    ...
    package static func fetchLatest(_ db: Database) throws -> [PeopleCard] {
        guard let window = try fetchLatestWindow(db) else { return [] }
        return try fetchForWindow(db, from: window.from, to: window.to)
    }

    package static func fetchByUser(_ db: Database, userID: String, limit: Int = 10) throws -> [PeopleCard] {
        try PeopleCard.fetchAll(
            db,
            sql: """
    ...
    package static func fetchAvailableWindows(_ db: Database) throws -> [(from: Double, to: Double)] {
    ...
    package static func fetchSummary(_ db: Database, from: Double, to: Double) throws -> PeopleCardSummary? {
