...
package enum FeedbackQueries {

    // MARK: - Write

    package static func addFeedback(
        _ db: Database,
        entityType: String,
        entityID: String,
        rating: Int,
    ...
    package static func getFeedback(
        _ db: Database,
        entityType: String,
        entityID: String
    ) throws -> Feedback? {
    ...
    package static func getStats(_ db: Database) throws -> [FeedbackStats] {
        guard try db.tableExists("feedback") else { return [] }
    ...
    package static func getAllFeedback(_ db: Database, limit: Int = 100) throws -> [Feedback] {
        guard try db.tableExists("feedback") else { return [] }
