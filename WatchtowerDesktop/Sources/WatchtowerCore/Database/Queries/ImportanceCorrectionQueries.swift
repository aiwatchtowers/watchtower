...
package enum ImportanceCorrectionQueries {
    ...
    package static func delete(_ db: Database, digestID: Int, decisionIdx: Int) throws {
        guard try db.tableExists("decision_importance_corrections") else { return }
        try db.execute(
    ...
    package static func correctedImportance(
        _ db: Database,
        digestID: Int,
        decisionIdx: Int
    ) throws -> String? {
    ...
    package static func allCorrections(_ db: Database) throws -> [String: String] {
        guard try db.tableExists("decision_importance_corrections") else { return [:] }
