...
package enum DayPlanQueries {

    // MARK: - Reads

    /// Fetch the plan for today's date (local time zone, "yyyy-MM-dd").
    package static func fetchToday(_ db: Database) throws -> DayPlan? {
    ...
    package static func fetchByDate(_ db: Database, date: String) throws -> DayPlan? {
        return try DayPlan.fetchOne(
            db,
            sql: "SELECT * FROM day_plans WHERE plan_date = ? ORDER BY id DESC LIMIT 1",
            arguments: [date]
    ...
    package static func fetchList(_ db: Database, limit: Int = 30) throws -> [DayPlan] {
    ...
    package static func fetchItems(_ db: Database, planId: Int64) throws -> [DayPlanItem] {
    ...
    package static func markItemDone(_ db: Database, itemId: Int64, cascadeToTask: Bool) throws {
        try db.execute(
            sql: """
    ...
    package static func markItemPending(_ db: Database, itemId: Int64, cascadeToTask: Bool) throws {
        try db.execute(
            sql: """
                UPDATE day_plan_items
    ...
    package static func markItemSkipped(_ db: Database, itemId: Int64) throws {
        try db.execute(
            sql: """
                UPDATE day_plan_items
    ...
    package static func deleteItem(_ db: Database, itemId: Int64) throws {
        try db.execute(
    ...
    package static func reorderBacklog(_ db: Database, planId: Int64, orderedIds: [Int64]) throws {
    ...
    @discardableResult
    package static func addManualItem(
        _ db: Database,
        planId: Int64,
