...
package enum UserStatsQueries {
    ...
    package static func fetchAll(_ db: Database) throws -> [UserStat] {
        let sql = """
            SELECT
                u.id,
                u.name,
    ...
    package static func setBotOverride(_ db: Database, userID: String, isBot: Bool?) throws {
        if let isBot {
            try db.execute(
    ...
    package static func setMutedForLLM(_ db: Database, userID: String, muted: Bool) throws {
        try db.execute(
    ...
    package static func fetchMessageStats(_ db: Database) throws -> (total: Int, human: Int, bot: Int) {
