...
package enum StatsQueries {
    /// Dashboard aggregate stats
    package static func fetchDashboardStats(_ db: Database) throws -> WorkspaceStats {
        try WorkspaceStats.fetch(db)
    }

    /// Message count per channel (for top channels display)
    package static func fetchTopChannels(_ db: Database, limit: Int = 10) throws -> [(channelID: String, name: String, count: Int)] {
    ...
    package static func fetchMessageVolume(_ db: Database, days: Int = 14) throws -> [(date: String, count: Int)] {
    ...
    /// Sync state summary
    package static func fetchSyncSummary(_ db: Database) throws -> (synced: Int, total: Int, totalMessages: Int) {
