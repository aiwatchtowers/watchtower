...
package enum SecretaryProfileQueries {
    /// Empty string if there is no workspace row yet, rather than throwing.
    package static func fetch(_ db: Database) throws -> String {
    ...
    package static func save(_ db: Database, text: String) throws {
        try db.execute(sql: "UPDATE workspace SET secretary_profile = ?", arguments: [text])
    }

    /// Empty strings when no workspace row exists yet.
    package static func fetchStyle(_ db: Database) throws -> (text: String, updatedAt: String) {
    ...
    package static func saveStyle(_ db: Database, text: String) throws {
        try db.execute(
