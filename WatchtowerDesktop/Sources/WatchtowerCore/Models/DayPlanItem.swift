...
package enum DayPlanItemKind: String, Codable {
    case timeblock
    case backlog
}

package enum DayPlanItemSourceType: String, Codable {
    case task
...
package enum DayPlanItemStatus: String, Codable {
    case pending
    case done
    case skipped
}
...
package struct DayPlanItem: FetchableRecord, Identifiable, Equatable {
    package let id: Int64
    package let dayPlanId: Int64
    package let kind: DayPlanItemKind
    package let sourceType: DayPlanItemSourceType
    package let sourceId: String?
    package let title: String
    package let details: String?          // maps to "description" column (avoids conflict with CustomStringConvertible)
    package let rationale: String?
    package let startTime: Date?
    package let endTime: Date?
    package let durationMin: Int?
    package let priority: String?         // "high", "medium", "low", or nil
    package let status: DayPlanItemStatus
    package let orderIndex: Int
    package let tags: String              // JSON array, e.g. "[]"
    ...
    package var isCalendarEvent: Bool { sourceType == .calendar }
    package var isManual: Bool { sourceType == .manual }

    /// Calendar items are read-only (sourced from external calendar, not editable).
    package var isReadOnly: Bool { isCalendarEvent }

    /// "HH:mm–HH:mm" formatted time range, nil if either start or end time is missing.
    package var timeRange: String? {
    ...
    package var isSkipped: Bool { status == .skipped }
