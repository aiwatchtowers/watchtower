...
final class FeatureManagerService {
    ...
    /// Enables one feature right now, outside the staged batch — the Inbox
    /// Strip's "Enable Reaction Commands" button. Same CLI call as `apply()`
    /// (`features enable <id>`, so the FEAT-03 fast-forward hook runs), same
    /// restart-then-reload tail and the same `isApplying`/`loadError` slots,
    /// but it never replays anything else staged in Settings → Features: a
    /// button on another screen must not apply decisions made (and not yet
    /// saved) there. A staged entry for this same id is superseded by the
    /// enable and dropped once it lands.
    ///
    /// It re-reads the live state first and does nothing unless the feature
    /// is actually disabled: `features enable` always runs its fast-forward
    /// hook, and for `reaction-commands` that hook seeds every reaction not
    /// yet polled as `skipped` — so re-enabling a feature someone already
