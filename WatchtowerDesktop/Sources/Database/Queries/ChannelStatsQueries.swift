...
enum ChannelStatsQueries {
    ...
    /// The owner's Slack user id (`OwnerQueries.resolve`), nil without one —
    /// channel stats match it against `messages.user_id`, so a Google- or
    /// Jira-keyed owner id would never match anything.
    static func fetchCurrentUserID(_ db: Database) throws -> String? {
        let slackUserID = try OwnerQueries.resolve(db).slackUserID
        return slackUserID.isEmpty ? nil : slackUserID
    }
