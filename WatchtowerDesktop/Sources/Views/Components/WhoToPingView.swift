...
struct WhoToPingView: View {
    ...
    @State private var slackLinks: SlackLinkResolver?
    ...
    private func openInSlack(_ target: PingTargetItem) {
        guard let url = Self.slackUserURL(target.slackUserID, links: slackLinks) else { return }
        NSWorkspace.shared.open(url)
    }

    /// The "Open in Slack" link for a (namespaced) user id. Static so the
    /// routing is testable without a view (the `IdeaDetailPane.mentionURL`
    /// precedent). An unloaded resolver still strips the namespace, just
    /// without a team.
    static func slackUserURL(_ userID: String, links: SlackLinkResolver?) -> URL? {
        let links = links ?? SlackLinkResolver(teamIDByAccount: [:], fallbackTeamID: "")
        return links.userURL(userID)
    }

    private func loadSlackLinks() async {
        do {
            slackLinks = try await Task.detached { [dbPool] in
                try dbPool.read { db in try SlackLinkResolver.load(db) }
            }.value
        } catch {
            slackLinks = nil
        }
