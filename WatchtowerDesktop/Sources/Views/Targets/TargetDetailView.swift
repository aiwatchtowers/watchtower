...
struct TargetDetailView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // chat, not the Details form — whether it is streaming, still
            // queued behind an earlier brief, or showing a failure banner.
            switch appState.targetBriefCenter.phase(for: target.id) {
            case .queued, .briefing, .failed:
        ...
        .onChange(of: appState.targetBriefCenter.phase(for: target.id)) { oldPhase, newPhase in
        ...
        .confirmationDialog(
            ...
            Button("Delete", role: .destructive) {
                ...
                // … and forget any brief still waiting for it.
    ...
    private func tabButton(_ tab: Tab) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 5) {
                ...
                // THIS target is queued or streaming (TargetBriefCenter).
                if tab == .assistant, appState.targetBriefCenter.isWorking(target.id) {
    ...
    private func briefFailureBanner(_ message: String) -> some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 0)
            Button {
                appState.targetBriefCenter.dismissFailure(targetID: target.id)
            } label: {
    ...
    private var assistantTab: some View {
        VStack(alignment: .leading, spacing: 0) {
            if case let .failed(_, message) = appState.targetBriefCenter.phase(for: target.id) {
    ...
    private func externalURL(_ ref: String) -> URL? {
        ...
        // `slack:<channelID>[:<ts>]` — archives link, or the generic channel
        // redirect for a channel-only ref (no team/domain needed either way).
        return SlackDeepLink.targetRef(ref)
    }
