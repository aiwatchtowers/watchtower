...
struct LogsSettings: View {
    ...
    enum LogFile: String, CaseIterable, Identifiable {
        ...
        /// match what it actually shows. The stdlib default logger and the
        /// Jira sub-loggers now write through the daemon's stream too
        /// (cmd/logfile.go's `routeStdLog`/`subLogger`), so what is left is
        /// code writing to stderr directly — the pid-file cleanup warnings.
        /// Wording mirrors the data-paths table in docs/daemon-pipeline.md
        /// (dual-path, task 5's commit 079fde23).
        var caption: String? {
            switch self {
            case .sync: nil
            case .daemon:
                "Go runtime panics, the launch-time rotation note, and pid-file cleanup warnings. "
                    + "Everything the daemon logs is in the Sync Log."
            }
        }
    }
