...
struct ProfileSettings: View {
    ...
    private func save() {
        ...
        let updated = UserProfile(
            id: profile?.id ?? 0,
            slackUserID: "", // keyed to the resolved owner by upsertOwnerProfile
            role: role,
            team: team,
        ...
        Task {
            do {
                try await db.dbPool.write { dbConn in
                    try ProfileQueries.upsertOwnerProfile(
                        dbConn, owner: OwnerQueries.resolve(dbConn), profile: updated
                    )
                }
                isSaving = false
            ...
            } catch OwnerError.noOwner {
                errorMessage = OwnerError.noOwner.localizedDescription
                isSaving = false
            } catch {
