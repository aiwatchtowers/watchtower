...
@MainActor
@Observable
final class WorkspaceOverviewViewModel {
    ...
    func load() async {
        ...
        do {
            ...
            let (ws, st, activity, links) = try await Task.detached { [dbManager] in
                try await dbManager.dbPool.read { db in
                    ...
                    return (ws, st, activity, try SlackLinkResolver.load(db))
                }
            }.value
            workspace = ws
            stats = st
