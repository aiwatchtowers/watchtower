...
final class OnboardingChatViewModel {
    ...
    private func saveProfileWithContext(_ contextText: String) async {
        ...
        do {
            ...
            // No owner yet (no account connected): park the answers on the
            // singleton row (ProfileQueries.noOwnerProfileKey) instead of
            // blocking onboarding — the first owner-scoped profile read/write
            // re-keys that row.
            try await dbManager.dbPool.write { db in
                let owner = try OwnerQueries.resolve(db)
                let key = owner.isKnown ? owner.id : try ProfileQueries.noOwnerProfileKey(db)
                let existingProfile = owner.isKnown
    ...
    func markOnboardingDone() async -> Bool {
        ...
        // as one — check it BEFORE the unknown-owner branch below, otherwise a
        // nil dbManager would fall into the local-completion path and mask the
        // failure as success.
        guard let dbManager else {
        ...
        do {
            ...
            // it, so a missing row can never mean a silently-unset flag. Read
            // and write share one transaction, so the whole-row rewrite cannot
            // clobber a concurrent saveProfileWithContext().
            try await dbManager.dbPool.write { db in
                let owner = try OwnerQueries.resolve(db)
                // No connected account yields an owner — a legitimate
                // completion, never a failure (returning `false` here is what
                // looped the feature-splash Retry forever, audit H2). The flag
                // lands on the singleton row under the no-owner key (the parked
                // onboarding answers, or a new pending row), which the first
                // owner-scoped read/write re-keys — so the completion survives
                // the owner's arrival. While the owner is still unknown the
                // DB flag is invisible to launch (`checkNeedsOnboarding` reads
                // no profile for an unknown owner), so until then only the
                // UserDefaults completion marker keeps onboarding from
                // re-running; a UserDefaults reset in that window shows it again.
                guard owner.isKnown else {
                    let key = try ProfileQueries.noOwnerProfileKey(db)
                    try db.execute(sql: """
                ...
                var profile = try ProfileQueries.fetchOwnerProfile(db, owner: owner)
                    ?? UserProfile(slackUserID: owner.id)
