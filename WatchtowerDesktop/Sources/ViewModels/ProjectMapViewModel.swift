        ...
        do {
            ...
            let (uid, reports) = try await Task.detached { [dbManager] in
                try dbManager.dbPool.read { db -> (String?, [String]) in
                    ...
                    // "Mine" matches Slack assignee ids, so the owner's Slack
                    // id — not the owner key, which may be google:/jira:.
                    let slackID = owner.slackUserID.isEmpty ? nil : owner.slackUserID
                    return (slackID, profile?.decodedReports ?? [])
                }
            }.value
            currentUserID = uid
            reportIDs = reports
