...
- **Memory rewrite/reflect stamp on attempt, not on success.** A page or workspace whose strong-tier step keeps failing now waits a full week for the next retry instead of being re-tried every ~70-minute cycle. This is deliberate — it is the cost this wave exists to cut, and a repeatedly-failing step retried every cycle was exactly the waste finding H2 named — but it is a two-line change plus one test to flip back to stamp-on-success if the owner would rather eat the retry cost than the wait.
