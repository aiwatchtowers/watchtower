...
5. **Backfill for real** — same command without `--dry-run`. One `memory(migrate)` commit plus a reindex. Re-running it is safe and does nothing.
   ...
   **If it is interrupted**, rewritten node files are already on disk with nothing committed. Do not start the daemon in that state: the next pipeline run would sweep them into a `memory(owner-edit)` commit (MEM-03), mis-attributing a machine migration to the owner. Re-run the command to completion — it is idempotent, and its commit picks up whatever the interrupted run had already staged. Then confirm the vault is clean before step 6: `git -C ~/.local/share/watchtower/<workspace>/memory status --short`. If anything is still listed (files the interrupted run wrote but never staged), commit it in the vault by hand as a `memory(migrate)` commit rather than leaving it for the daemon.
