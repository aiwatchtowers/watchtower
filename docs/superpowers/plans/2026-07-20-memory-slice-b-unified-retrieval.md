...
A snapshot is already prepared this session at a scratch location, taken via `sqlite3 .backup` from the live `<corp-workspace>` workspace (2,062 memory nodes, 465k messages) — never touching the live daemon's database. Before running anything:
