...
- [ ] **Step 3: Live migration check on a COPY of the real DB** — `cp ~/.local/share/watchtower/<corp-workspace>/watchtower.db /tmp/wt-migrate-test.db` then open it with a scratch config pointing at the copy (`WATCHTOWER_CONFIG_PATH` is Desktop-only; for Go use a temp config yaml with a temp workspace dir containing the copy) and assert: account #1 exists with the real email, counts of `gmail_messages` unchanged, `inbox_items` gmail rows rewritten. Never run against the live DB (the daemon may hold it).
