...
Before the first Swift build in this worktree: `WatchtowerDesktop/.build` does not exist here. Copy it from the main checkout to avoid the ~5-minute cold ML build — `cp -R <main-checkout>/WatchtowerDesktop/.build WatchtowerDesktop/.build` (only if absent). Then `cd WatchtowerDesktop && swift build` once so incremental builds are fast.
