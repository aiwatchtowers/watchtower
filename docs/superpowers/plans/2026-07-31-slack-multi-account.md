...
- [ ] **Step 3: Live migration check on a COPY of the real DB** — `cp ~/.local/share/watchtower/<corp-workspace>/watchtower.db /tmp/wt-slack-migrate-test.db`, open it with a scratch config pointing at the copy, run the migration, assert: `slack_accounts` has one row with the real team info; `channels`/`messages`/`users` row counts unchanged; a spot-check message's `channel_id`/`user_id` now carry the `"1:"` prefix; `inbox_items` gmail-prefixed rows untouched; `digests`/`tracks`/`people_cards` row counts unchanged. Never run against the live DB (the daemon may hold it).
