...
- The three sites are the complete set: `grep -rn '"<@"' --include="*.go" internal/` plus the two `fmt.Sprintf`/concatenation forms in `internal/db` were checked; no other code embeds a user id in mention markup.
