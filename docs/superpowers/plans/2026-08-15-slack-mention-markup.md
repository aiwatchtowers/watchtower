...
`messages.text` stores what Slack sent: `<@<owner-slack-id>>`, and no migration rewrites it — it is source data. Migration 00048 namespaced `slack_accounts.current_user_id` to `1:<owner-slack-id>`, and every site that builds mention markup from that id now searches for `<@1:<owner-slack-id>>`, which cannot exist.
