...
- **Never run the `watchtower` binary against the owner's live workspace** (`~/.local/share/watchtower/<corp-workspace>`) and never open its SQLite database. Tests use their own temp DBs / temp vaults.
