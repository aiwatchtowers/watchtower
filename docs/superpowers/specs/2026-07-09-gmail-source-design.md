...
- **Google verification:** `gmail.modify` — restricted scope. Для личного/командного
  ...
  - **Пересмотрено 2026-07-09:** перед подачей на верификацию scope сужен до
    `gmail.readonly` — код (`internal/gmail/client.go`) реально делает только
    `messages.list`/`messages.get` (read-only), запись/модификация нигде не
    вызывается, поэтому `gmail.modify` был шире фактического использования.
    Trade-off: План 3 (write-back) при реализации потребует расширения scope
    обратно до `gmail.modify` и повторной авторизации/верификации у всех
    подключённых пользователей — это сознательно принято ради более быстрой и
    дешёвой (без security assessment) верификации сейчас.
