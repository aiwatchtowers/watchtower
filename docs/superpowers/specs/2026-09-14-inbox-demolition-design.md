...
| `inbox_feedback`, `feed_items`, `feed_state` | `DROP TABLE` | Only readers were the dead Dashboard and memory's interaction ingest (§5.3). `inbox_feedback` has 0 rows on the live install. (There is no `situation_feedback` table — situation-level thumbs live in the general `feedback` table as `entity_type='situation'` rows, which stay as frozen history.) |
