...
- Both paths create the row **mechanically** via the existing `TargetQueries.create` (level "day", `source_type` per prefill or "manual", `parentID` honored for "Add sub-target", `secondaryLinks` honored). ⌘Enter stops there. Enter additionally hands off to `TargetBriefCenter` (§9.6) and navigates to the new target's detail with the Assistant tab active.
