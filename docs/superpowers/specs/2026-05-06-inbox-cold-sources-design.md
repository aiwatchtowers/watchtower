...
User report: "only briefings are landing in the inbox". Diagnosis against the live `<corp-workspace>` workspace DB (689 MB) revealed three independent causes, all of which stop legitimate signals from reaching the UI.
...
3. **Jira identity gap.** `workspace.current_user_id = <owner-slack-id>` (Slack `@<corp-domain>`); `jira_user_map` keys by Atlassian account_id, joined by email; the user's Atlassian email is `@<corp-domain>`. There is no mapping row for `<owner-slack-id>`. `DetectJira` is passed Slack ID and matches against `jira_issues.assignee_account_id` — never matches.
