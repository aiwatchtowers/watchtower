...
Once per GATED consolidation run — final-review Fix 2, 2026-07-26: matching runs on EVERY gated call, not only when focus.md's text changed, so a standing bullet naming a not-yet-existing node stops being permanently inert once that node shows up — parse `focus.md` → directives; for each bullet, resolve mechanically:
...
- title match against `memory_nodes.title`: a case-insensitive substring match performed in Go (`strings.Contains(strings.ToLower(title), strings.ToLower(fragment))`) against one `ListMemoryNodeTitles()` fetch per matching pass (final-review Fix 3: SQLite's `lower()` folds ASCII only and silently misses non-Latin titles — the owner works in Russian — so folding case in Go is required for correctness, and fetching titles once per pass rather than once per SQL query per candidate is also cheaper).
...
`importance_score` refreshes only for touched nodes (MEM-16 mechanics); a focus edit touches no node files, so scores would go stale. Fix: a fingerprint of the RESOLVED MATCH SET (sorted node ids, tagged `now:`/`cooled:`, hashed — `matchFingerprint`, note: final-review fix 2026-07-26) persists in a workspace column (`memory_focus_fingerprint TEXT`, migration 00031 — runtime state, MEM-02-exempt like the watermarks). Fingerprinting the resolved set rather than focus.md's raw text (the original v1 design) matters because the set can change even when the file doesn't: a brand-new node that a standing bullet now resolves to must trigger a sweep too, not just an owner edit to the file. When the current fingerprint differs from the stored one, that run performs a refine pass over ALL indexed nodes' importance (reusing the MEM-16 phase-B refinement machinery) and then stores the new fingerprint — only after the sweep succeeded. This freezes on a STEP-level error (an alias/title lookup failure, or the sweep's own node-list read failing) — the fingerprint stays put and the next run retries the same work; a per-node sweep failure (one node's file missing/corrupt since indexed) is instead quarantined — logged, skipped, and counted — and does NOT freeze the fingerprint, since the whole-vault sweep must still make progress on every other node.
...
`memory.focus.enabled` (default **false**). Off → the file is never parsed, `Focus` is always none (multiplier 1.0). **Gate-off neutralization (final-review Fix 1, 2026-07-26):** the gate's else-branch calls `runFocusDisable` rather than skipping the step entirely, because a workspace that had the gate ON at some point and accumulated `memory_focus_matches`/boosted `importance_score`s must not keep that skew forever once the owner flips it back off. `runFocusDisable` reads the stored fingerprint: empty (never enabled, or already neutralized by a prior disabled run) takes a fast path — zero DB writes, byte-identical to a never-enabled workspace; non-empty means residual state exists, so it empties `memory_focus_matches`, sweeps every indexed node's importance back to its un-boosted value, and only then clears the fingerprint (same freeze-on-error discipline as the gate-on sweep). A never-enabled workspace is therefore still byte-identical run over run. The Desktop Focus section stays visible regardless of the gate (editing a file the pipeline ignores is harmless and lets the owner prepare focus before enabling).
