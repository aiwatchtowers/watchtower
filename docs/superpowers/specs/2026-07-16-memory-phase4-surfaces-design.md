...
- **Floor-advance discipline (by-design):** the chat-turn floor advances only after the belief pass completes to a clean commit AND was not cut short by the `beliefs_max` cap. A belief-pass error, a budget-skip, or a cap-break holds the floor so the same owner turns re-stage next run (a cap-break may leave a staged owner ref uncited). A pass that completed *without* a cap-break advances the floor even when the model declined to cite any staged ref — the turn had its chance and is not re-scanned forever; it may still be re-derived later if the owner restates it or edits the vault. (Reviewed 2026-07-16, M3.)
