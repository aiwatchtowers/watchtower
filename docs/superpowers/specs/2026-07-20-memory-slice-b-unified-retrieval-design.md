...
- Real-data verification: not an automated test — a one-time, human-readable report from running `retrieve-compare` against the owner's workspace snapshot, reviewed as part of this slice's final task before any switch.
