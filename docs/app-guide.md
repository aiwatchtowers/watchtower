...
Watchtower is a macOS desktop app that syncs your Slack workspace to a local database and uses AI to generate insights: daily briefings, a secretary dashboard (situations awaiting your attention), calendar integration with meeting prep, digests, tracks, and people analytics.
...
**Needs Attention** — Items requiring your immediate focus: open dashboard situations, updated tracks, overdue/blocked tasks, new red flags from people cards. Each item shows priority, reason, and links to its source (dashboard situation, track, digest, task, or person). Items may include `suggest_task` — a prompt to create a task from a track that needs action. Click to navigate.
...
**Feed — one secretary-ranked list:** Situations are ordered by rank (priority + recency), not split into visual tiers. Each row shows:
- **Kind badge** — `Signal` (clustered Slack/Jira/Calendar signals), `Target` (a target update), `Track` (a track update), or `Mixed` (a situation combining more than one kind).
- A priority icon and a one-line summary (falls back to the AI's triage reason if no card yet).
- Expanding a row shows the full context packet — **Why it matters**, **Summary**, **Chronology** — plus the original member signals (sender, channel, snippet), loaded on first expand. While the card is still being generated, the row shows "Preparing context…"; if card generation failed, it shows "Context unavailable — will retry" and the pipeline retries it next cycle.

**Actions** (on the expanded row, or via right-click for quick access): **Done**, **Dismiss**, **Snooze** (1 hour / Till tomorrow / Till Monday), 👍/👎 feedback, plus **Target** and **Track** buttons.
...
**Profile tab** — a free-text brief you write about yourself (role, priorities, who/what matters), stored as `workspace.secretary_profile` and injected into the triage, composer, and situation-card prompts so the secretary's judgment reflects your actual context, not just message content.
...
**Sidebar** — Shows a compact "next event" indicator with time and title when calendar is connected. The Calendar tab icon is "calendar" in the sidebar between Dashboard and Tasks.
...
| **Dashboard** | "Target" button on a dashboard situation |
| **Manual** | "+" button in the Tasks toolbar or sidebar |
| **Chat** | AI suggests creating a task during conversation |

Each task created from a source retains a link back to it. Click the source link in the detail view to navigate to the originating track, digest, briefing, or dashboard situation.
...
8. **Maintenance** — unsnoozes tasks and dashboard situations whose snooze period has passed, returning them to active status
...
- **Dashboard situations** — Clusters of signals (Slack/Jira/Calendar) plus target/track updates, composed into a single narrative unit with its own context packet (summary, why it matters, chronology). This is what the Dashboard tab's feed shows, ranked by priority and recency.
