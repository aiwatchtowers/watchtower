...
**Which tasks the assistant can touch** — a task's chat can also propose changes to that task's **sub-tasks at any depth and its parents**, not just the task you opened it on; a card that lands on another task says which one. It can never reach a task off that line — a sibling, or anything unrelated — and such a proposal fails with an explanation instead of writing somewhere unexpected. When you give it a direct instruction it may apply changes to **the task you are looking at** without asking; anything aimed at another task always waits for your **Approve**, however plainly you asked for it.
