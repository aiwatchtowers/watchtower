...
**Meetings** — transcription settings grouped into Engine (engine and model pickers, language set), Recording (live transcription, cross-window context, mic auto-gain, preload model before meetings, auto-record on join, retention days), Speakers (speaker roles toggle, diarization threshold), and an Advanced disclosure (window size, confidence threshold, margin, force language).
