...
- **Transcription is fully local** (on-device; the model downloads once on first use). The default is **WhisperKit with Large v3 Turbo** — faster and lighter than full Large v3 while staying multilingual. You can switch providers (WhisperKit, Parakeet, Qwen3, or Apple) and models in Settings › Transcription › Engine and Model pickers. Mixed ru/uk/en meetings are handled by per-window language detection with a sticky fallback (WhisperKit only; other providers are batch-only with no live transcription). Only the transcript text is sent to the AI provider for the recap.
