...
**Feed — master-detail screen:** The left panel shows a compact ranked list (priority + recency), with each row displaying a kind badge (`Signal` / `Target` / `Track` / `Mixed`), priority icon, and one-line summary. The right panel appears when you select a row, showing the full context for that situation: **Why it matters**, **Summary**, **Chronology** (generated by the AI secretary); below that, a **Sources** block listing the situation's member signals (Slack messages, Jira items, calendar events) as clickable cards with per-message Slack deep links, and a **Members** section with user badges. While the context packet is being generated, the panel shows "Preparing context…"; if generation failed, it shows "Context unavailable — will retry" and the pipeline retries it next cycle.

The bottom action bar offers: 👍 and 👎 buttons, a "Comment to teach the secretary…" text field (submitting feedback with a comment teaches learned rules; a bare thumb is processed locally without AI), **Snooze** (1 hour / Till tomorrow / Till Monday), **Target** (create a task from this situation), **Track** (create a track), **Dismiss**, and **Done** (Enter key). Clicking **Done** or **Dismiss** marks the situation handled and automatically selects the next situation in the list.
