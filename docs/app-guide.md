...
**Settings** — The Google Calendar section in Settings shows connection status and a "Sync days ahead" picker (3/5/7/14 days) when connected. Config fields: `calendar.sync_days_ahead` (default: 2), `calendar.history_days` (past days kept synced and shown in the Meetings list; default: 14).
...
**Slack Workspaces:** Connect any number of Slack workspaces side by side, each with its own OAuth authorization and its own synced data. The list shows one row per workspace (its label or team name, its `<domain>.slack.com`, and a status dot — green connected, orange needs attention/error, red revoked with the error as a tooltip). Each row has an **Enabled** toggle (a disabled workspace stops syncing but keeps all its already-synced data), a **Re-login** button (shown when the workspace needs re-authorization), and **Remove**. **Add Slack Workspace** opens a small sheet with an optional label; **Connect** launches Slack's authorization page in your browser, and after you approve, the workspace is added and syncing starts. **Removing a workspace is non-destructive** — it disconnects the workspace (drops its authorization) but keeps its already-synced messages, digests, and situations in Watchtower. The older single-account **Disconnect** button in the Workspace block above now behaves the same way: it drops the Slack authorization and stops syncing, but keeps all already-synced Slack data.
...
**Daemon:** Read-only status (running/stopped, binary path, and any daemon start/stop error) — the Start/Stop toggle is gone; lifecycle is app-owned. See "Menu Bar & App Lifecycle" below.
