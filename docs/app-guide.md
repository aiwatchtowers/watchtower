...
**System** — Workspace (active workspace, read-only), AI (provider — Claude or Codex, model, workers, Claude/Codex CLI paths with auto-detection indicator, Test Connection), Sync (poll interval, workers, initial history days, sync threads), Daemon (read-only status — running/stopped, binary path, and any daemon start/stop error — the Start/Stop toggle is gone; lifecycle is app-owned, see "Menu Bar & App Lifecycle" below), Data (storage info, regenerate AI data, delete everything), Logs (live daemon logs with filtering), Update (check/download/install app updates), and a link to Usage & Pipeline Progress.
