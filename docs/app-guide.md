...
- **Transcription is fully local** (WhisperKit, on-device; the model downloads once on first use). The default is **Large v3 Turbo** — faster and lighter than full Large v3 while staying multilingual; you can switch models in Settings › Transcription. Mixed ru/uk/en meetings are handled by per-window language detection with a sticky fallback. Only the transcript text is sent to the AI provider for the recap.
