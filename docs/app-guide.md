...
- **Speaker names (voice prints):** once you rename a speaker in a transcript (see the Transcript tab below), Watchtower learns that person's voice locally. In future recordings a recognized voice is labeled with the person's name automatically instead of "Speaker N" — `[Я]` detection always wins over a voice match. Voice prints never leave your machine and are learned only from your manual renames, never automatically.
