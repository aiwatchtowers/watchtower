...
On the **Scopes** step, ensure exactly these three are present (add via "Add or
remove scopes" if missing):

- `.../auth/calendar.events.readonly` (sensitive)
- `.../auth/calendar.calendarlist.readonly` (sensitive)
- `.../auth/gmail.readonly` (restricted)

Do **not** add broader scopes — narrower scope = faster review. In particular,
do not add `gmail.modify` until Plan 3 (write-back) actually ships.
