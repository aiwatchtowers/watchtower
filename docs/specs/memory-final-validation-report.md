...
| M8 | **major (integration gap, found in the 2026-07-18 owner drill)** | the dashboard's situation-level 👍/👎 (the owner's primary rating gesture since bda8032) persists to `feedback(entity_type='situation')` on BOTH paths (Swift fast path and `watchtower inbox feedback`), while the memory interaction ingest is floor-driven on `inbox_feedback` (per-item) — owner ratings never reach memory's engagement/`act:` machinery | extend `action_ingest` with a second floor-driven source over `feedback('situation')` (subject mapping already exists via the verdict path), or migrate the ingest to the `feedback` table; NEEDS-OWNER-REVIEW on which table is authoritative |
