...
2. **Desktop, ~5 minutes, with `memory.sources.actions` on:** ~~👎 one situation~~ (**superseded by M8** — situation-level thumbs don't reach `inbox_feedback`; re-drill after the M8 fix decides the authoritative table), ~~dismiss/done one situation~~ (**done 2026-07-18** — verdict path validated on a genuine `user_done`, see 3b), **convert** one situation to a Target (still open — exercises the `## Links` conversion cross-link on the target's mirror, with `memory.sources.operational` also on).
