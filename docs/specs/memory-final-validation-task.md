...
> **Living document** — each phase appends its section as it lands on `feature/secretary-memory`. The owner runs this ONCE, when the whole memory feature is implemented, as the last gate before PR #36 leaves draft. Run on the work machine (live workspace `<corp-workspace>`).
...
The vault at `~/.local/share/watchtower/<corp-workspace>/memory/` already contains the phases 0–2 E2E data (447+ entities, ~384+ episodes incl. 8 known duplicate episodes from the killed-run incident, one manually-set watermark) — this is deliberately the starting state: phase 3 must cope with a lived-in vault, not a fresh one.
...
All four gates default false (`memory.sources.operational`, `memory.surfaces.day_plan`, `memory.surfaces.meeting_prep`, `memory.semantic.preferences`). First confirm the **dark-default invariant**: with all four off, one `consolidate` + one day-plan generation + one `meeting-prep` behave identically to post-slice-3 EXCEPT that the day-plan and meeting-prep prompts are byte-identical to the **v3/v4-with-sentinel** render — the bumped default templates themselves carry the new sentinel-bearing `MEMORY OPEN LOOPS` / `MEMORY` sections, so the gate-off prompt is not byte-identical to slice-3's v2/v3 templates (it is the new template rendered with its sentinel). The run-done log shows `mirrors: 0 mirrored`, no `target:`/`track:` entity is created, and the belief-pass user message IS byte-identical to slice 3's for identical inputs (the `memory.revise_beliefs` template is unbumped — the OWNER ACTIONS block rides the user message, absent when the gate is off). Then run the drills.
