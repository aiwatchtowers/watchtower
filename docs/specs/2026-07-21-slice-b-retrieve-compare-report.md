...
> Date: 2026-07-21. Real-data verification of the Slice B unified retrieval mechanism (`docs/superpowers/specs/2026-07-20-memory-slice-b-unified-retrieval-design.md`), run against a safe, read-only snapshot of the live `<corp-workspace>` workspace (2,062 memory nodes at snapshot time, 465k messages). The snapshot and vault clone live at a session-scratch path outside the repo; never the live daemon's database. See methodology below for exactly how isolation was achieved.
...
- **"colleague C"**: the dropped legacy hit is a short episode *entirely* about her (title, story, and both participants are her and one colleague). The new top hit is a large multi-topic "product rollout" situation episode where she appears in exactly one bullet line among ~10 named contributors — a genuine keyword match (confirmed the term appears in the body), but far less central to a query specifically about her.
- **"colleague D"**: same pattern — the dropped hit is a 1:1 "leadership 1:1" meeting with just him and the owner; the new top hit is a 12-participant "Task sync" where he's one of many attendees.
...
**Methodology caveat (found by the final whole-branch review, confirmed independently):** the `coverage_ok` metric itself is methodologically pessimistic — `legacyIDs` is the live handler's actual alias+FTS combination, but `newIDs` comes from `RetrieveByQuery` alone, which never resolves aliases (by design — the switched handler is meant to keep prepending the alias hit ahead of it, per design spec §2). So the aggregate 80% figure understates what a real switch would actually preserve for any query whose only legacy hit was an alias match. This does **not** change the two spot-checked cases above: `watchtower memory open "colleague C"` and `"colleague D"` both confirm neither string resolves as a registered alias, so both queries' comparisons were pure FTS-vs-FTS on both sides — the relevance-regression finding stands independent of this metric bias. The bias affects the aggregate coverage percentage, not the substantive finding; recorded as a known limitation in `docs/inventory/memory.md` for any future re-verification pass.
