...
**Observable:** Every write-tool call that reaches the chat-mode MCP server becomes one `agent_actions` row and nothing else. With trust `ask` (the default) no data table changes on the call; the tool's `Execute` runs only from `Registry.Apply`, which the Desktop drives after the owner approved. A validation failure writes no row at all. This holds identically on the **runtime-B in-process loop** (`internal/agentloop`, the Go tool loop for HTTP providers): a write-tool call there routes through `Registry.Propose` (records a proposal), never `Execute` — the loop has no path that executes a write tool.
...
- 2026-09-14 (**inbox demolition**, spec `docs/superpowers/specs/2026-09-14-inbox-demolition-design.md`): **no contract semantics changed, no guard relaxed.** The two read tools the 2026-09-05 runtime-B entry below added — `list_situations` and `get_situation` (`internal/tools/situations.go`) — are **removed** from the registry and from `readtools.go`, so neither the runtime-B loop nor the MCP dev surface mounts them any more: handing an agent a table frozen on 2026-09-06 as "what is going on" is audit finding L4. `Registry.CallRead` and every other read tool are untouched, and `TestBuildToolRegistry_PinsWriteToolsReadToolsAndSurfaces` was updated to the shorter read-tool list — it still pins the exact set, so a tool can neither be added nor dropped without the pin moving. The write tools, their surfaces and their trust rules are byte-unchanged. AGENT-04's list of draft-only surfaces still includes the situation chat in this PR; the `SituationChatViewModel` and its `toolModes == [nil]` assertion are removed in the Desktop half of the demolition, which narrows the list without changing the rule. **Desktop half landed:** `SituationChatViewModel` and its test are gone, and AGENT-04's Observable/Test-guards lines above are updated to drop the situation chat accordingly.
...
- 2026-09-05: runtime B (`internal/agentloop`) lands — the Go tool loop for OpenAI-compatible HTTP providers (the ollama chat provider on the main/target surfaces). AGENT-01's Observable and guard list extended to it: a write-tool call in the loop routes through `Registry.Propose` (new read twin `Registry.CallRead` serves read tools without a proposal row), never `Execute`. Slice 1 adds two read tools to the registry (`list_situations`, `get_situation`) for the loop; the MCP read handlers are untouched (AGENT-02 unchanged), so the two faces coexist until the full read-tool migration.
