...
## TGT-BRIEF-01 — Secretary, not assistant

**Status:** Enforced

**Observable:** The target brief chat stays on the secretary side of the secretary/assistant line, along three axes:
...
**Observable:** A `watchtower-action` block without `"mode":"execute"` — mode absent, `"propose"`, or any unknown value — is **never auto-applied**: it renders as a pending proposal card behind the existing Approve gate, byte-for-byte today's behavior. Execute mode exists **only** in the Desktop target chat (`TargetChatViewModel` parse → `TargetActionExecutor` apply, the same single Swift writer the Approve button uses); the Go track-watcher grammar (`internal/prompts/defaults.go`, `defaultTrackRun`) stays a propose-only subset — it is never taught `"mode"` or the four new kinds, so old `track_events` rows and old model output decode as propose. A malformed execute-mode block is surfaced as an error and skipped, never applied.
...
**Test guards:**
- `WatchtowerDesktop/Tests/ProposedActionTests.swift` (mode leniency: absent/unknown ⇒ propose)
