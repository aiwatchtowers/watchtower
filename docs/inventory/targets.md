...
- 2026-09-24 (audit fix wave 5b, owner decision 16): `TargetBriefCenter` became a FIFO queue instead of a single slot — a second Enter-create now queues behind a still-running brief instead of cancelling it (`startBrief` and `markFailed` both used to `cancelStream()` the running run), and failures are per target, staying visible until that target's banner is dismissed. **TGT-BRIEF-02 is strengthened, not weakened:** the row is still created mechanically before any hand-off (`CreateTargetSheet.submit` ordering untouched), and a superseded brief used to end with a truncated reply, no cards and *no visible error at all* — the queue closes that hole, so "a failed brief leaves the row plus a visible chat error" now holds for the second-Enter case too. **TGT-BRIEF-01 axis 2 (Timing) is unaffected:** a brief that starts forty seconds later is still one-shot, still originates in the owner's own Enter, is neither scheduled nor a re-run — axis 2 forbids daemon-driven and unsolicited turns, not delayed dispatch of the owner's single instruction; intent origin (axis 1) and write reach (axis 3) are untouched. No contract text changed. The four view sites that read the center's scalar `phase` moved onto a new per-target `phase(for:)` (the scalar stays as the head projection, the `MeetingRecorderCenter.phase` precedent); there is deliberately no Retry affordance and no queue cap — the owner recovers a failed brief by re-asking in the chat (spec §7), and each queue entry costs one keystroke.
- 2026-09-14 (audit fix wave 5, decision 12 reading (a)): documented, not changed — the four Reaction Commands Wave 2 tools (`create_track`, `create_idea`, `remind_me`, `brief_context`) had already gone `Surfaces: ["reaction"]` in `d7cfc1a9` (2026-09-12, PR #152 review, before this wave started), so they were never reachable from the target or main chat despite carrying no `target_id` scoping of their own. Axis 3's mechanism paragraph above now names the registry `Surfaces` gate as a second enforcement mechanism alongside apply-time `TargetTreeScope`, closing the omission (nothing in TGT-BRIEF-01..03 was ever contradicted by the code). See `docs/inventory/reaction-commands.md`'s 2026-09-12 changelog entry for why the four tools moved to reaction-only.
