...
Reaction Commands is the sixth hook and the one that does not stamp a watermark, because it has none: `reactions.list` carries no time filter, so a poll always re-enumerates the owner's whole recent reaction history and the `reaction_commands` ledger is the only thing that makes a re-poll quiet. Its hook therefore *records* that history instead — every reaction the owner has already placed is written to the ledger as `skipped` with the detail `"seeded on enable (pre-existing reaction)"` (`internal/reactioncmd.SeedLedger`), so the first poll after the enable dispatches nothing. Without it, enabling the feature replayed the backlog as commands, three of whose dictionary emoji carry `execute` trust and would have applied inline rather than proposed. Two consequences follow from the same fail-closed ordering as every other hook: **enabling this feature needs Slack reachable** (an unreachable account, or an enabled account whose token file is missing, fails the hook and leaves the feature off — a partially seeded enable would arm exactly the account whose history replays), and the hook needs a live Slack client, which `internal/features` cannot build. That capability is injected through `FastForward`'s `Deps` seam, populated by `cmd/features.go`; the hook table stays the single dispatch point.
