...
This applies to all gated phases: `phaseChannelDigests`, `phaseCustomTrackScan`, `phaseTracksAndRollups` (tracks arm), `phasePeopleCards`, `phaseInbox`, `phaseStreamDigests`, `phaseIdeas`, `phaseMemory`, `phaseNextStep`, `phaseBriefing`, `runDayPlanPhase`, `runDayPlanConflictPhase`. The gate check must come first, before any pipeline-related logic. (`phaseFastInbox` and `phaseFeed` were on this list until 2026-09-14, when both were deleted with the surfaces they served — see the changelog.)
...
**Test guards:**
- `internal/daemon/daemon_gates_test.go::TestFeatureGates_DisabledPhaseWritesNoPipelineRun` (12-case table: digests, inbox, tracks, tracks_starved_without_digest, people, people_starved_without_digest, ideas, stream_digests, memory, next_step, briefing, day_plan; asserts no `pipeline_runs` rows and no `ideas_backfill.lock` for ideas; each phase gate tested with its feature flag disabled. The 13th case, `feed`, was dropped on 2026-09-14 with `phaseFeed` itself — one case fewer because one phase fewer, not because a gate stopped being checked)
...
Reaction Commands is the sixth hook and the one that does not stamp a watermark, because it has none: `reactions.list` carries no time filter, so a poll always re-enumerates the owner's whole recent reaction history and the `reaction_commands` ledger is the only thing that makes a re-poll quiet. Its hook therefore *records* that history instead — every reaction the owner has already placed is written to the ledger as `skipped` with the detail `"seeded as pre-existing reaction history"` (`internal/reactioncmd.SeedLedger`; the string said "seeded on enable" until 2026-09-26, when the first poll became a second seed site), so the first poll after the enable dispatches nothing. Without it, enabling the feature replayed the backlog as commands, three of whose dictionary emoji carry `execute` trust and would have applied inline rather than proposed. Two consequences follow from the same fail-closed ordering as every other hook: **enabling this feature needs Slack reachable** (an unreachable account, or an enabled account whose token file is missing, fails the hook and leaves the feature off — a partially seeded enable would arm exactly the account whose history replays), and the hook needs a live Slack client, which `internal/features` cannot build. That capability is injected through `FastForward`'s `Deps` seam, populated by `cmd/features.go`; the hook table stays the single dispatch point.
...
- `internal/features/fastforward_test.go::TestFastForward_ReactionCommands` (the injected seed runs on enable; a seed failure — and an unwired seeder — fails the hook)
- `internal/reactioncmd/seed_test.go::TestReactionCmd_SeedRecordsHistoryWithoutDispatching` (the seed records every pre-existing owner reaction, dictionary or not, and the first poll afterwards makes zero AI calls and creates zero `agent_actions`/`ideas` rows)
- `internal/reactioncmd/seed_test.go::TestReactionCmd_NewReactionAfterSeedDispatchesExactlyOne` (the seed closes history without deafening the feature)
- `internal/reactioncmd/seed_test.go::TestReactionCmd_SeedFailsWhenAnAccountCannotBeReached` (an enabled Slack account the resolver could not reach fails the seed rather than being silently left unseeded)
...
- 2026-09-14 (**inbox demolition**, spec `docs/superpowers/specs/2026-09-14-inbox-demolition-design.md`): **no contract changed and no guard relaxed** — the registry shrinks, the rules over it do not. The `dashboard` and `feed` entries are **deleted** (both described the removed surface; neither id was referenced outside the registry and its tests), and `phaseFastInbox`/`phaseFeed` leave FEAT-01's gated-phase list along with the `feed` case in its guard table. `secretary-inbox` **keeps its id** — it is a stable identifier used by `features enable secretary-inbox` and the onboarding first-contact latch — but is re-described as **"Attention detection"** with `Cost: CostNone` (it makes no AI calls at all any more), no `SubToggles`, and `FeedsInto: ["briefing"]`; its fast-forward hook keeps only `SetInboxLastProcessedTS` now that the composer's `compose_last_run_ts` has no reader, which leaves FEAT-03 ("enable resumes from now") intact on the one watermark that still matters. `slack-digests` no longer lists `secretary-inbox` in its `FeedsInto` — that edge was false even before the demolition (digests never fed detection) and its removal only makes the FEAT-04 cascade dialog honest; `stream-digests → secretary-inbox` stays real, since Jira comment sync is what feeds the Jira-comment-mention detector. `memory` loses its `sources.actions` and `surfaces.disputes` sub-toggles. **FEAT-02 is unaffected and stays literally true:** the demolition does drop three tables (`inbox_feedback`, `feed_items`, `feed_state`) and freeze `situations` to `stale`, but that is a migration the owner installs, not something a feature *disable* does — no gate anywhere deletes data, and `situations`, ideas, digests, targets and tracks all remain queryable.
...
- 2026-09-13 (audit fix wave 5, owner decision 6): FEAT-03 extended with a sixth fast-forward hook, `reaction-commands`. It is the first hook with no watermark to stamp — it seeds the `reaction_commands` ledger with the owner's pre-existing reactions instead — and the first to need a capability `internal/features` cannot build, which is why `FastForward` grew a `Deps` seam (`cmd/features.go` populates it; `cmd/memory.go` passes the zero value). The principle is unchanged: enabling never replays history. No new contract number was minted.
- 2026-09-26 (owner call): `reaction-commands` defaults to **on**. FEAT-03's "enabling never replays history" now holds for an install that never ran `features enable` at all: `internal/reactioncmd.Pipeline` seeds an account with an empty `slack_accounts.reaction_commands_seeded_at` stamp (migration 00073) on its first poll and dispatches only from the next one; the enable hook is unchanged and shares the stamp. Changelog and guards in `docs/inventory/reaction-commands.md`.
