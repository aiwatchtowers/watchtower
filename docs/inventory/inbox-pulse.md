...
- 2026-08-16 (mention backfill CLI): `watchtower inbox backfill-mentions --since <YYYY-MM-DD> [--dry-run]` (`cmd/inbox.go`, wrapping `Pipeline.BackfillMentions` in `internal/inbox/backfill.go`) recovers @mentions a detector missed during a known dead window by scanning only mentions, per enabled Slack account, from an explicit `--since` timestamp instead of the shared cursor. It deliberately does **not** read or write `inbox_last_processed_ts` — INBOX-09 is untouched by design, since rewinding the shared watermark would re-process everything else (DMs, thread replies, ordinary channel traffic) that occurred in the same window just to recover a handful of mentions. Backfilled items land untriaged in the conservative `actionable` class per INBOX-01's default, and `FindPendingMentions`' existing dedup guard (`NOT EXISTS` against `inbox_items`) makes re-running the command over the same window safe. The command shares `createItemsFromCandidates` with the regular per-cycle detector, so it inherits that function's known grouping limitation: candidates are grouped by (channel, thread) and the closing-signal filter runs against the group's latest message only, so a "thanks" arriving after a missed mention can suppress recovery of the whole group — the backfill is not a guaranteed full recovery. No contract number changes.
