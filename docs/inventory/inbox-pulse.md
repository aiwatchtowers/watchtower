...
- 2026-08-08 (Ideas & Decisions Registry): the dormant `jira_comment_mention` trigger and its INBOX-02 auto-resolve went live. Migration 00050 adds a real `jira_comments` table (bounded per-account comment sync), so `DetectJira`'s comment-mention branch and `autoResolveJira` finally have data to read. Identity resolution is via `jira_user_map` (slack id → Atlassian account id): a Jira `[~mention]` embeds the ATLASSIAN id, not the Slack one, so an unmapped owner is a graceful no-op rather than a miss. The INBOX-02 fixtures were adapted to the real `jira_comments` schema and now write Jira Cloud's own dotted-millisecond timestamp format, exactly as the sync does — **assertions unchanged**. That fixture change exposed a real production bug in `autoResolveJira`: it string-compared `jira_comments.created_at` (Jira format) against `inbox_items.created_at` (RFC3339), where '.' sorts below 'Z', so a comment had to be a whole second newer to register at all and one in the same second never would. Both comparisons now go through `db.FormatJiraTime`/`db.ParseJiraTime`. No contract number changes.
  - **Owner decision needed — feature coupling:** the bounded Jira comment sync is wired only when `ideas.enabled = true` (`cmd/sync.go`'s `SetCommentSyncLimit`). With the ideas registry off, `jira_comments` stays empty, so this INBOX-02 path and the `jira_comment_mention` trigger are silently inert — an inbox behavior gated on an unrelated feature's switch. Flagged, not resolved: whether comment sync should have its own switch (or ride `inbox.enabled`) is the owner's call. **Resolved 2026-08-12 — see below.**
- 2026-08-12 (Decisions Split + Cross-Source Digests, design `docs/superpowers/specs/2026-08-12-decisions-split-cross-source-digests-design.md`): the coupling flagged above is resolved. Jira comment sync (`cmd/sync.go`'s `SetCommentSyncLimit`) now gates on a new `streams.enabled` config key (default **true**), not `ideas.enabled`. `streams.enabled` also gates the Gmail/Jira stage-1 stream-digest passes themselves (`internal/ideas.Pipeline.RunStreamDigests`, its own daemon phase `phaseStreamDigests`), independently of whether stage-2 consolidation (`ideas.enabled`) is on. With the default on, `jira_comments` sync — and with it the `jira_comment_mention` trigger and its INBOX-02 auto-resolve — are now live by default, decoupled from the Ideas registry's own switch. No contract number changes.
- 2026-08-15 (Slack mention-markup fix): `FindPendingMentions` was inert from migration 00048 (2026-07-31) until this fix — `slack_accounts.current_user_id` became namespaced (`"1:U123"`), but `messages.text` still carries Slack's raw mention markup untouched, so the `LIKE` patterns built straight from the namespaced id matched nothing and the @mention inbox trigger silently produced zero candidates for the whole window. The same bug hit two sites outside this module — `internal/db/channel_stats.go`'s per-channel mention count and `internal/tracks/pipeline.go`'s mention-based channel score — noted here since it's the same class of bug found by the same investigation. Fixed by reducing the id to its raw form before building markup, behind one shared seam (`slack.MentionPatterns`/`slack.MentionTag`, `internal/slack/namespace.go`) rather than three separate ad hoc patches. No contract number changes.
- 2026-08-15 (Slack per-account inbox detection): the four trigger detectors (`FindPendingMentions`, `FindPendingDMs`, `FindThreadRepliesToUser`, `FindReactionRequests`) were still account-blind after the mention-markup fix above — namespaced `channel_id`/`user_id` columns existed, but no query filtered by account and only a single identity was ever excluded. With two connected Slack organizations that produced three concrete defects: a mention in one account's channel could leak into the other account's results (the raw `<@U…>` markup carries no account discriminator, so an unscoped `LIKE` match against one account's identity also matched an identical mention sitting in a different connected account's channel — pinned by `TestFindPendingMentions_ScopedToAccount`), every account's DMs were returned while only account #1's id was excluded from them (the owner's own outgoing DMs in account 2 looked incoming), and thread replies/reactions never saw account 2's messages at all. Fixed by adding an `accountID` parameter and an `m.channel_id LIKE ? || ':%'` predicate to all four (`internal/db/inbox.go`), and looping the new `detectSlackAccounts` (`internal/inbox/pipeline.go`) over `ListEnabledSlackAccounts()`, calling `detectSlackTriggers` once per account with that account's own `current_user_id`; per-account errors are joined so one account's failure never silently drops a sibling account's results within the same cycle. `autoResolveSlack` (INBOX-02) had the same latent gap once accounts 2+ started producing real items — every pending Slack item was checked against a single pinned identity — and `mention`/`dm`/`thread_reply` items are fixed the same way: each item's account is derived from its own `channel_id` prefix and checked against that account's own `current_user_id`. `reaction` items are not covered by either the old or the new code, and never have been: `autoResolveSlack`'s switch matches a `"reaction_request"` trigger type no row has ever held — `FindReactionRequests` sets `"reaction"`, which the switch does not list, and the `inbox_items.trigger_type` CHECK does not even permit `"reaction_request"` — a gap predating this branch, left uncorrected here since changing it is a behavior change under the Enforced INBOX-02 contract and the owner's call. The per-workspace `inbox_last_processed_ts` cursor is deliberately unchanged: nothing observed required forking it per account. No contract number changes.
- 2026-08-16 (mention backfill CLI): `watchtower inbox backfill-mentions --since <YYYY-MM-DD> [--dry-run] [--force]` (`cmd/inbox.go`, wrapping `Pipeline.BackfillMentions` in `internal/inbox/backfill.go`) recovers @mentions a detector missed during a known dead window by scanning only mentions, per enabled Slack account, from an explicit `--since` timestamp instead of the shared cursor. It deliberately does **not** read or write `inbox_last_processed_ts` — INBOX-09 is untouched by design, since rewinding the shared watermark would re-process everything else (DMs, thread replies, ordinary channel traffic) that occurred in the same window just to recover a handful of mentions. Backfilled items land untriaged in the conservative `actionable` class per INBOX-01's default. `--since` is UTC midnight and exclusive (Go's `"2006-01-02"` layout parses with no zone, so `time.Parse` returns UTC; the pipeline's `ts_unix > sinceTS` is a strict `>`), and a `--since` more than 90 days back is rejected unless `--force` is passed, so a mistyped year cannot silently sweep the whole `messages` table.
  - **Deliberately its own create-only path, not a reuse of the regular detector's.** An earlier version of this command called `createItemsFromCandidates`, the same grouped/fold-capable path `detectSlackTriggers` uses per detection cycle — built for a window of minutes, where two of its semantics are safe assumptions that stop being true over a multi-week backfill window. Caught in review before release: (1) grouping by `(channel, thread_ts)` keeps only the latest message per group, and a plain (non-threaded) message's `thread_ts` is always `""`, so every plain-channel mention across the whole window collapsed into one item per channel — recovering roughly one mention out of however many actually went missing; (2) folding into "any pending item already on the thread" doesn't check that item's age, so a multi-week-old candidate could fold into a currently live, unrelated conversation sharing the same thread, silently rewriting its `message_ts` backwards and — via `UpdateInboxItemSnippet` — wiping its `ai_reason`/`read_at` and risking an unwanted re-compose; both were invisible in `--dry-run`, since the fold branch's guard `continue`d before any counter. Fixed by giving the backfill its own loop (`backfillAccountMentions`): no grouping, and `UpdateInboxItemSnippet`/`MergeWaitingUserIDs` are never called from this path — every candidate becomes its own `CreateInboxItem` call, full stop. `createItemsFromCandidates`/`detectSlackTriggers` (the live per-cycle path) were not touched.
  - **Idempotency is now actually true, not aspirational.** With create-only semantics, `FindPendingMentions`' existing dedup guard (`NOT EXISTS` against `inbox_items`, keyed on `channel_id`+`message_ts`) is on its own sufficient to make re-running the command over the same window safe any number of times — every recovered candidate becomes its own row instead of being merged away, so a second run over a multi-mention thread creates nothing further and loses nothing already recovered (the grouped/fold path could not make this claim: a second run over an under-recovered channel would fold an older sibling into the one item that did get created, degrading it one step further rather than recovering it).
  - **The backfill does not apply closing-signal filtering at all** — a trailing "thanks @me" becomes its own recovered item unless the owner already replied after it, a deliberate, reviewer-judged-defensible difference from the live per-cycle detector (which does filter closing signals, per-message-latest-in-group, per `TestPipeline_ClosingSignalSkipped`), not a limitation carried over from it. What the create-only path does skip: a candidate the owner already reacted to or replied to *after* (`CheckUserReplied` — the same "handled in Slack already" check `autoResolveSlack`/INBOX-02 uses), counted `already_answered` in the envelope rather than recovered. This is deliberately **not** `CheckUserRepliedBefore` (the closing-signal pre-filter's own check, which tests for a prior message *before* the given timestamp): applied to a mention's own timestamp instead of a trailing closing signal's, that would ask "did the owner post anything in this channel before being mentioned," true almost by default in any channel the owner is active in, and would have wrongly suppressed most plain-channel recoveries.
  - No contract number changes.
- 2026-09-25 (owner identity resolver, spec `docs/superpowers/specs/2026-09-25-no-slack-owner-identity-design.md`): the `jira_assigned` trigger is live for the first time. Before this branch `DetectJira` matched `jira_issues.assignee_account_id` (an Atlassian id) against the owner's Slack id, so it never fired; it now matches the owner's Atlassian id (`db.Owner.JiraAccountID`, with the Slack id kept only as the pre-resolver fallback). Because every edit of an assigned issue — the owner's own included — bumps `updated_at`, the `(issue key, updated_at)` dedup alone would mint a fresh pending Catch-Up item per update; the detector additionally skips an issue key that already has a **pending** `jira_assigned` item, so there is at most one pending item per assigned issue. A resolved or dismissed item does not block a later update from surfacing a new one, and neither does a stale-archived one (`ArchiveStaleActionable` sets `archived_at` but leaves `status='pending'`, so the check also requires `archived_at IS NULL`). INBOX-02's auto-resolve (`autoResolveJira`: the owner's own comment on the issue after the item was created resolves it) is unchanged, and no guard assertion changed. No contract number changes.
