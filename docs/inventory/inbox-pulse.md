...
- 2026-08-16 (mention backfill CLI): `watchtower inbox backfill-mentions --since <YYYY-MM-DD> [--dry-run] [--force]` (`cmd/inbox.go`, wrapping `Pipeline.BackfillMentions` in `internal/inbox/backfill.go`) recovers @mentions a detector missed during a known dead window by scanning only mentions, per enabled Slack account, from an explicit `--since` timestamp instead of the shared cursor. It deliberately does **not** read or write `inbox_last_processed_ts` — INBOX-09 is untouched by design, since rewinding the shared watermark would re-process everything else (DMs, thread replies, ordinary channel traffic) that occurred in the same window just to recover a handful of mentions. Backfilled items land untriaged in the conservative `actionable` class per INBOX-01's default. `--since` is UTC midnight and exclusive (Go's `"2006-01-02"` layout parses with no zone, so `time.Parse` returns UTC; the pipeline's `ts_unix > sinceTS` is a strict `>`), and a `--since` more than 90 days back is rejected unless `--force` is passed, so a mistyped year cannot silently sweep the whole `messages` table.
  ...
  - **The closing-signal caveat that used to apply here is gone, replaced by a narrower, correctly-scoped skip.** The old (pre-fix) version inherited `createItemsFromCandidates`' known limitation where the closing-signal filter runs only against a group's latest message, suppressing a whole group. Grouping no longer exists in this path, so that caveat does not apply. What the create-only path does skip: a candidate the owner already reacted to or replied to *after* (`CheckUserReplied` — the same "handled in Slack already" check `autoResolveSlack`/INBOX-02 uses), counted `already_answered` in the envelope rather than recovered. This is deliberately **not** `CheckUserRepliedBefore` (the closing-signal pre-filter's own check, which tests for a prior message *before* the given timestamp): applied to a mention's own timestamp instead of a trailing closing signal's, that would ask "did the owner post anything in this channel before being mentioned," true almost by default in any channel the owner is active in, and would have wrongly suppressed most plain-channel recoveries.
  - No contract number changes.
