...
**Observable:** Acknowledging a recap (`Pipeline.Acknowledge` / CLI `catchup ack`, or the Desktop's "I'm caught up" button writing the shared DB directly) marks read everything **inside its window** on the five `read_at` surfaces — `digests` (`type='channel'` and `period_to > from AND period_from < to`), `stream_digests` (the same overlap on ISO `period_from`/`period_to`), `tracks` (`updated_at` in `(from, to]` and not dismissed; also clears `has_updates`), `inbox_items` (`created_at` in `(from, to]`), `briefings` (`date` between the window's local dates) — plus stamps the recap's own `acknowledged_at` (first stamp wins). All six writes run in one transaction, set-based. Items outside the window are left untouched, and a second ack changes nothing (every predicate already excludes the rows it marked). Selection is by **window**, not by the refs the compose call happened to cite: an item the model dropped for lacking a valid ref, or content the AI simply never mentioned, still gets marked read — "caught up" means "up to date until T," not "reviewed everything the AI surfaced."

The two **summary** surfaces (`digests`, `stream_digests`) use the OVERLAP predicate rather than a `period_to <= to` containment, because a summary covers a span, not an instant, and because the run's own coverage top-up generates digests that stamp their own `period_to` with their own `time.Now()` — routinely a second or two past the window's `to`. The gather (`ListCatchupDigests`/`ListCatchupStreams`) already uses overlap, so the recap cites those digests; a containment ack would leave them unread forever and the badge would lie. `CatchupCoverage` reads the same overlap, so a reported reach may sit slightly past `to`. `tracks`/`inbox_items` carry a single instant, so they stay `(from, to]`.
...
- 2026-09-04 (final review): CATCHUP-01's two **summary** surfaces switched from the containment predicate `period_to > from AND period_to <= to` to the gather's OVERLAP predicate `period_to > from AND period_from < to`, on both paths (`AcknowledgeCatchupWindow`, `CatchUpQueries.acknowledge`) and in `CatchupCoverage`. The old predicate leaked: `Pipeline.Run` samples the recap's `to` *before* `runTopUp`, and the digest/stream pipelines stamp each new row's `period_to` with their own `time.Now()`, so a top-up digest routinely lands a second past `to` — cited by the recap (the gather already used overlap) yet never marked read and never counted in coverage. The `digests` statement also gained the `type='channel'` filter the gather has always had, so a daily/weekly roll-up is no longer collateral. Guards extended in place: `TestCatchup01_AcknowledgeMarksWindowRead`, `TestCatchupCoverage`, `testAcknowledgeMarksWindowReadOnFiveSurfaces`, `testAcknowledgeLeavesItemsOutsideWindowUnread`. No contract weakened — the window is the same window; the change is which rows count as "inside" it.
