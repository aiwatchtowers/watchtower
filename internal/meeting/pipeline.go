...
func (p *Pipeline) prepareForEvent(ctx context.Context, event db.CalendarEvent, userNotes string) (*MeetingPrepResult, error) {
	...
	// Owner info: an unknown owner (or a resolve error) keeps "User" and no
	// profile context.
	owner, _ := p.db.ResolveOwner()
	userName := "User"
	...
	profileCtx := ""
	if profile, err := p.db.GetOwnerProfile(owner); err == nil && profile != nil {
