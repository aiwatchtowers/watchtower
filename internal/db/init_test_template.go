...
func InitTestTemplate() error {
	...
	ddl, gooseRows, reactionMapRows, err := extractTemplateDDL()
	if err != nil {
		return err
	}
	openMemoryHook = func() (*DB, error) {
		return buildCloneDB(ddl, gooseRows, reactionMapRows)
	}
	return nil
}

// reactionMapRow is one migration-seeded reaction_command_map row (00063) — the
// default emoji dictionary. The DDL-only extraction below drops data rows, so
// these are carried explicitly into every clone — the same way
// goose_db_version rows are.
...
// statements, goose version rows, and migration-seeded reaction_command_map
// rows needed to clone that schema cheaply.
func extractTemplateDDL() (ddl []string, gooseRows [][2]int64, reactionMapRows []reactionMapRow, err error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("querying reaction_command_map rows: %w", err)
	}
	defer rrows.Close()
	...
	for rrows.Next() {
		...
		if err := rrows.Scan(&r.emoji, &r.kind, &r.tool, &r.handlerID, &r.enabled); err != nil {
			return nil, nil, nil, fmt.Errorf("scanning reaction_command_map row: %w", err)
		}
		reactionMapRows = append(reactionMapRows, r)
	}
	if err := rrows.Err(); err != nil {
		return nil, nil, nil, fmt.Errorf("iterating reaction_command_map rows: %w", err)
	}

	return ddl, gooseRows, reactionMapRows, nil
}
...
// migration-seeded reaction_command_map rows.
func buildCloneDB(ddl []string, gooseRows [][2]int64, reactionMapRows []reactionMapRow) (*DB, error) {
