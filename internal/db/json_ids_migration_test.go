...
func TestMigration00054_TracksChannelIDs(t *testing.T) {
	...
	if _, err := raw.Exec(`INSERT INTO tracks (id, text, channel_ids) VALUES
		(1, 'bare pair', '["C0FAKE63","C0FAKE64"]'),
		(2, 'already namespaced', '["1:C0FAKE63"]'),
...
func TestMigration00054_TracksParticipants(t *testing.T) {
	...
	if userID0 != "1:U0FAKE05" {
		t.Errorf("participants[0].user_id = %q, want 1:U0FAKE05", userID0)
	}
	...
	if userID1 != "1:U0FAKE06" {
		t.Errorf("participants[1].user_id = %q, want 1:U0FAKE06 (already namespaced, untouched)", userID1)
	}
