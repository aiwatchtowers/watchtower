...
func TestGetJiraIssuesByAssigneeAccountID(t *testing.T) {
	db := openTestDB(t)
	SeedTestJiraAccount(t, db)

	seed := func(key, statusCat, accountID, priority string) {
		require.NoError(t, db.UpsertJiraIssue(JiraIssue{
			AccountID: 1,
			Key:       key, ProjectKey: "P", Summary: "Summary " + key,
			Status: "S", StatusCategory: statusCat,
			AssigneeAccountID: accountID, AssigneeSlackID: "U1", Priority: priority,
			Labels: `[]`, Components: `[]`,
			CreatedAt: "2026-04-01T00:00:00Z", UpdatedAt: "2026-04-01T12:00:00Z", SyncedAt: "2026-04-01T12:00:00Z",
		}))
	}
	seed("P-1", "todo", "acc-me", "Low")
	seed("P-2", "in_progress", "acc-me", "High")
	seed("P-3", "done", "acc-me", "Highest") // excluded: done
	seed("P-4", "todo", "acc-other", "High") // excluded: someone else

	issues, err := db.GetJiraIssuesByAssigneeAccountID("acc-me")
	require.NoError(t, err)
	require.Len(t, issues, 2)
	assert.Equal(t, "P-2", issues[0].Key, "priority order, High first")
	assert.Equal(t, "P-1", issues[1].Key)
}
