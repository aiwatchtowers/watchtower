...
func (db *DB) GetJiraIssuesByAssigneeSlackID(slackID string) ([]JiraIssue, error) {
	return db.openIssuesAssignedBy("assignee_slack_id", slackID)
}

// GetJiraIssuesByAssigneeAccountID returns non-done issues assigned to an
// Atlassian account id, ordered like GetJiraIssuesByAssigneeSlackID.
func (db *DB) GetJiraIssuesByAssigneeAccountID(accountID string) ([]JiraIssue, error) {
	return db.openIssuesAssignedBy("assignee_account_id", accountID)
}

// GetOwnerAssignedJiraIssues returns the owner's non-done assigned issues: by
// their own Atlassian account id when known, else by their Slack id, else
// none. It never queries with an empty key — assignee columns default to the empty string,
// so an empty key would match every unmapped issue in the workspace.
func (db *DB) GetOwnerAssignedJiraIssues(o Owner) ([]JiraIssue, error) {
	switch {
	case o.JiraAccountID != "":
		return db.GetJiraIssuesByAssigneeAccountID(o.JiraAccountID)
	case o.SlackUserID != "":
		return db.GetJiraIssuesByAssigneeSlackID(o.SlackUserID)
	default:
		return nil, nil
	}
}

// openIssuesAssignedBy is the shared body of the two assignee readers; column
// is one of their two fixed column names, never caller input.
func (db *DB) openIssuesAssignedBy(column, value string) ([]JiraIssue, error) {
	rows, err := db.Query(`SELECT `+jiraIssueColumns+`
		FROM jira_issues
		WHERE `+column+` = ? AND status_category != 'done' AND is_deleted = 0
		ORDER BY CASE priority
			WHEN 'Highest' THEN 1 WHEN 'High' THEN 2 WHEN 'Medium' THEN 3
			WHEN 'Low' THEN 4 WHEN 'Lowest' THEN 5 ELSE 6 END,
			updated_at DESC`, value)
	if err != nil {
		return nil, fmt.Errorf("querying jira issues by %s %s: %w", column, value, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		issue, err := scanJiraIssue(rows)
		if err != nil {
			return nil, fmt.Errorf("scanning jira issue by %s: %w", column, err)
		}
		issues = append(issues, issue)
	}
	return issues, rows.Err()
