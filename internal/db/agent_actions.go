...
// MaxAgentActionID returns the highest agent_actions id, 0 for an empty table.
func (db *DB) MaxAgentActionID() (int64, error) {
	var id int64
	if err := db.QueryRow(`SELECT COALESCE(MAX(id), 0) FROM agent_actions`).Scan(&id); err != nil {
		return 0, fmt.Errorf("reading max agent action id: %w", err)
	}
	return id, nil
}
