...
//
// Swift twin (a deliberate dual path — change both sides together):
// SlackLinkResolver in
// WatchtowerDesktop/Sources/WatchtowerCore/Utilities/SlackDeepLink.swift. One
// documented divergence: when the resolved team is empty, Go still emits a
// `team=` deep link while Swift returns nil (the view hides the button).
