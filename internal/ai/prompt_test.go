...
// search_knowledge anchors hold namespaced channel ids and a channel-day hit
// has no single ts, so the prompt must say how to build a link from a hit.
// Swift twin: ChatViewModel.knowledgeLinkRule.
func TestBuildSystemPrompt_KnowledgeHitLinkRule(t *testing.T) {
	prompt := BuildSystemPrompt("test-ws", "test-ws", "T001", "schema", "")

	assert.Contains(t, prompt, `prefer the hit's "link"`)
	assert.Contains(t, prompt, `without its "N:" account prefix`)
	assert.Contains(t, prompt, "chunk_anchor")
