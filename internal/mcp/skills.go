...
type loadSkillArgs struct {
	Name string `json:"name" jsonschema:"skill name exactly as listed in the SKILLS block (lowercase letters, digits and dashes)"`
}
...
// gates what the SKILLS block lists, not what the read returns, so a model
...
func registerSkills(s *mcpsdk.Server, skillsDir string) {
	mcpsdk.AddTool(s, &mcpsdk.Tool{
		...
		Description: "Load one persona skill by name — the full instructions for handling a class " +
			"of request. Call it when a skill listed in the SKILLS block matches what the owner " +
