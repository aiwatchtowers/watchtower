...
type Daemon struct {
	...
	noOwnerLoggedDay map[string]string
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	for {
		...
		case <-ticker.C:
			d.runSync(ctx)
		// Both watchers close their channel once ctx is cancelled, and a closed
		// channel is always ready in a select — without the ok check the loop
		// would keep drawing "events" and re-entering runSync on the dead ctx
		// until the random select pick landed on ctx.Done(). A closed watcher
		// is dropped (nil blocks forever) so only ctx.Done() remains.
		case _, ok := <-d.wakeChannel():
			if !ok {
				d.wakeCh = nil
				continue
			}
...
func (d *Daemon) runSync(ctx context.Context) {
	...
	// minutes) — never on a cancelled ctx (shutdown), like every pipeline.
	if ctx.Err() == nil {
	...
	d.runDayPlanPhases(ctx, time.Now())
}
...
// kbCycleBudget bounds one cycle's indexing so the first Slack backfill
// spreads across cycles instead of delaying every later phase.
const kbCycleBudget = 60 * time.Second

// phaseKnowledgeIndex brings the knowledge-search index up to date.
// Mechanical (no AI, KB-02); off = no indexing, the existing index stays
// readable (FEAT-01/02).
	...
	d.trackedPipelineRun("knowledge-index", func() pipelineRunStats {
		...
		if err != nil {
			if isBenignShutdownErr(ctx, err) {
				// Daemon shutdown mid-cycle, not a real failure: cursors are
				// tx-safe (runSource.go saves a cursor only with the batch
				// that advanced it), so the next cycle resumes cleanly. Don't
				// stamp "context canceled" onto the pipeline_runs row as
				// status "error" — the phaseJiraSync precedent for the same
				// shape.
				d.logger.Printf("knowledge index: stopped by shutdown")
				reportErr = nil
			} else {
...
// done — a daemon shutdown mid-cycle — rather than a genuine failure. kb.Run
// joins per-source errors via errors.Join, so a cancellation that occurred
// after a real source error already landed comes back as both in one joined
// error; only a result where EVERY leaf is the cancellation/deadline error is
// benign; a genuine error joined alongside the cancel must still be reported.
func isBenignShutdownErr(ctx context.Context, err error) bool {
...
// calls). reaction_commands.interval_hours > 0 throttles; the default (0)
// polls every cycle — a reaction is a command the owner is waiting on, so the
// poll rides every tick and every `Sync Now`, and runs right after the Slack
// sync, before the other source syncs and the AI pipelines (runSync). Two
// costs of that placement, accepted: a revoked-but-present Slack token fails
// reactions.list and records an error pipeline_runs row every cycle (the
// phaseSlackSync shape), and a backlog burst spends up to maxDispatchPerRun
// compose calls before Calendar/Gmail/Jira sync. Detection is idempotent (the
// ledger, REACT-03), so an in-memory throttle is enough — a restart simply
// re-polls and re-dispatches nothing already seen.
func (d *Daemon) phaseReactionCommands(ctx context.Context) {
	if !d.config.ReactionCommands.Enabled {
		return
	}
	if d.reactionCmdPipe == nil || d.db == nil {
		return
	}
	// A Slack-less install (Google/Jira-only — the feature is on by default)
	// has nothing to poll: skip before the tracked run so no 0-item
	// pipeline_runs row lands every cycle (the phaseSlackSync precedent).
	accounts, err := d.db.ListEnabledSlackAccounts()
	if err != nil {
		d.logger.Printf("reaction-commands: listing slack accounts: %v", err)
		return
	}
	if len(accounts) == 0 {
		return
...
func (d *Daemon) phaseBriefing(ctx context.Context) {
	...
	if d.briefingPipe == nil || !d.shouldRunBriefing() || !d.briefingHasOwner(time.Now()) {
		return
	}
	d.trackedPipelineRun("briefing", func() pipelineRunStats {
		id, err := d.briefingPipe.Run(ctx)
		if errors.Is(err, db.ErrNoOwner) {
			// No owner is a benign skip: no budget, one line a day. The
			// briefingHasOwner gate normally catches it before this run is
			// tracked; this covers a daemon with no DB of its own.
			d.logNoOwnerOnce(time.Now(), "briefing")
			err = nil
		}
		if err != nil {
			// A real failure (AI generate/parse/store error) reached this
			// point — RunForDate's benign skips (no owner, handled above;
			// "no data yet", (0, nil)) never consume budget; only a real
			// error does.
...
// applyInboxOwner hands the inbox pipeline this cycle's owner (the resolver
// does the Slack → Google → Jira fallback, so Calendar/Gmail detection works
// on an install without Slack). It reports false when the phase should skip:
// no owner identity is a benign skip, logged once per UTC day. A lookup error
// clears the pipeline's owner without logging, so Run resolves again and
// reports the failure once — logged as the phase's error and recorded in
// pipeline_runs — rather than running on last cycle's owner. No DB → Run
// resolves from its own.
func (d *Daemon) applyInboxOwner(now time.Time) bool {
	if d.db == nil {
		return true
	}
	owner, err := d.db.ResolveOwner()
	if err != nil {
		d.inboxPipe.SetOwner(db.Owner{})
		return true
	}
	if !owner.Known() {
		d.logNoOwnerOnce(now, "inbox")
		return false
	}
	d.inboxPipe.SetOwner(owner)
	return true
}
...
// cycle, so a lookup error is logged once per cycle, not once per phase.
func (d *Daemon) runDayPlanPhases(ctx context.Context, now time.Time) {
	if !d.config.DayPlan.Enabled || d.dayPlanPipeline == nil || d.db == nil {
		return
	}
	owner := d.dayPlanOwner(now)
	d.runDayPlanPhase(ctx, now, owner)
	d.runDayPlanConflictPhase(ctx, now, owner)
}
...
// a known owner, no plan yet for today.
func (d *Daemon) shouldRunDayPlan(now time.Time, owner db.Owner) bool {
	...
	existing, _ := d.db.GetDayPlan(owner.ID, date)
	return existing == nil
}
...
func (d *Daemon) runDayPlanPhase(ctx context.Context, now time.Time, owner db.Owner) {
	if !d.config.DayPlan.Enabled {
		return
	}
	if !d.shouldRunDayPlan(now, owner) {
		return
	}
	if d.db == nil {
		return
	...
	d.trackedPipelineRun("day_plan", func() pipelineRunStats {
		plan, err := d.dayPlanPipeline.Run(ctx, dayplan.RunOptions{UserID: owner.ID, Owner: owner, Date: date})
		items := 0
		if plan != nil {
			items = 1
		}
		...
		if err != nil {
			// shouldRunDayPlan already filtered out the "no owner"
...
func (d *Daemon) runDayPlanConflictPhase(ctx context.Context, now time.Time, owner db.Owner) {
	if !d.config.DayPlan.Enabled {
		return
	}
	if d.dayPlanPipeline == nil || d.db == nil || !owner.Known() {
		return
	}
	userID := owner.ID
	date := now.Format("2006-01-02")
