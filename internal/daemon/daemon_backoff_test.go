...
func TestDaemon_DayPlanBackoff_ThreeFailuresExhaustBudget(t *testing.T) {
	...
	d.runDayPlanPhase(context.Background(), testTime, d.dayPlanOwner(testTime))
	d.runDayPlanPhase(context.Background(), testTime.Add(time.Hour), d.dayPlanOwner(testTime.Add(time.Hour)))
	d.runDayPlanPhase(context.Background(), testTime.Add(2*time.Hour), d.dayPlanOwner(testTime.Add(2*time.Hour)))
	...
	d.runDayPlanPhase(context.Background(), testTime.Add(3*time.Hour), d.dayPlanOwner(testTime.Add(3*time.Hour)))
...
func TestDaemon_DayPlanBackoff_LogsGivingUpOnceBudgetExhausted(t *testing.T) {
	...
	d.runDayPlanPhase(context.Background(), testTime.Add(3*time.Hour), d.dayPlanOwner(testTime.Add(3*time.Hour)))
	d.runDayPlanPhase(context.Background(), testTime.Add(4*time.Hour), d.dayPlanOwner(testTime.Add(4*time.Hour)))
...
func TestDaemon_DayPlanBackoff_BenignNoUserSkipDoesNotConsumeBudget(t *testing.T) {
	...
	database := db.OpenTestDB(t)
	// Deliberately no account seeded: ResolveOwner() is unknown, so
	// shouldRunDayPlan must refuse before ever calling Run.
	...
	testTime := time.Date(2026, 4, 23, 8, 0, 0, 0, time.Local)
	for i := 0; i < 5; i++ {
		d.runDayPlanPhase(context.Background(), testTime.Add(time.Duration(i)*time.Hour), d.dayPlanOwner(testTime.Add(time.Duration(i)*time.Hour)))
	}
...
func TestDaemon_DayPlanBackoff_ResetsNextCalendarDay(t *testing.T) {
	...
	d.runDayPlanPhase(context.Background(), day1, d.dayPlanOwner(day1))
	d.runDayPlanPhase(context.Background(), day1.Add(time.Hour), d.dayPlanOwner(day1.Add(time.Hour)))
	d.runDayPlanPhase(context.Background(), day1.Add(2*time.Hour), d.dayPlanOwner(day1.Add(2*time.Hour)))
	require.Equal(t, 3, fp.runCalls)
	d.runDayPlanPhase(context.Background(), day1.Add(3*time.Hour), d.dayPlanOwner(day1.Add(3*time.Hour)))
	...
	d.runDayPlanPhase(context.Background(), day2, d.dayPlanOwner(day2))
	assert.Equal(t, 4, fp.runCalls, "a new calendar day must get a fresh budget and actually launch a real attempt")
	d.runDayPlanPhase(context.Background(), day2.Add(time.Hour), d.dayPlanOwner(day2.Add(time.Hour)))
	assert.Equal(t, 5, fp.runCalls, "day 2's 2nd attempt must also launch — its budget must not have inherited day 1's spent count")
	d.runDayPlanPhase(context.Background(), day2.Add(2*time.Hour), d.dayPlanOwner(day2.Add(2*time.Hour)))
	...
	// be refused again.
	d.runDayPlanPhase(context.Background(), day2.Add(3*time.Hour), d.dayPlanOwner(day2.Add(3*time.Hour)))
...
func TestDaemon_DayPlanBackoff_SurvivesRestart(t *testing.T) {
	...
	testTime := time.Date(2026, 4, 23, 8, 0, 0, 0, time.Local)
	d1.runDayPlanPhase(context.Background(), testTime, d1.dayPlanOwner(testTime))
	d1.runDayPlanPhase(context.Background(), testTime.Add(time.Hour), d1.dayPlanOwner(testTime.Add(time.Hour)))
	d1.runDayPlanPhase(context.Background(), testTime.Add(2*time.Hour), d1.dayPlanOwner(testTime.Add(2*time.Hour)))
	...
	d2.runDayPlanPhase(context.Background(), testTime.Add(3*time.Hour), d2.dayPlanOwner(testTime.Add(3*time.Hour)))
...
func TestDaemon_BriefingBackoff_BenignNoUserSkipDoesNotConsumeBudget(t *testing.T) {
	...
	database := db.OpenTestDB(t)
	// Deliberately no owner: RunForDate returns db.ErrNoOwner before Generate
	// is ever called, and phaseBriefing treats it as a benign skip.
