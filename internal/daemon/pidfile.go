...
//
// The warnings here and in FindProcess write to stderr directly rather than
// through a logger: both are shared with CLI commands (sync stop, status, kb)
// that have no daemon logger, they fire only on a failed remove, and the
// daemon process itself reaches only RemovePID, once at shutdown — so the few
// lines that land in daemon.log for a detached child cannot grow it.
