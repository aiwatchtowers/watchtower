...
// generated yet). Thin adapter over db.GetBriefing for the owner.
func NewGetTodayBriefing() *Tool {
	return &Tool{
		...
		Execute: func(_ context.Context, d *db.DB, _ Call) (any, error) {
			// No owner is an error, not null: null means "not generated yet"
			// (OWNER-02). Read-only either way (DEV-01).
			owner, err := d.RequireOwner()
			if err != nil {
				return nil, fmt.Errorf("getting owner: %w", err)
			}
			today := time.Now().Format("2006-01-02")
			briefing, err := d.GetBriefing(owner.ID, today)
			if err != nil {
