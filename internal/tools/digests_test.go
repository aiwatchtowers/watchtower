...
// TestOwner02_GetTodayBriefingToolErrorsWithoutOwner pins OWNER-02 on the MCP
// surface: with no connected account there is no owner whose briefing could
// be "today's", so the tool errors with db.ErrNoOwner instead of answering
