...
// `watchtower reaction-commands list` distinguishes "closed as history" from
...
// Both seed sites — the enable hook and the first poll — write it.
const seedDetail = "seeded as pre-existing reaction history"
...
// seedAccount records one account's whole reaction history as seen and stamps
// the account (slack_accounts.reaction_commands_seeded_at), which is what the
// poll keys its own first-poll seed on. It reuses FilterUnseenReactionCommands
// so re-running the hook (a second enable after a disable) only adds what
// arrived since, and InsertReactionCommand's INSERT OR IGNORE keeps even that a
// no-op. The stamp is written LAST: a failure part-way leaves the account
// unstamped, so the next poll seeds again rather than dispatching the rest.
func seedAccount(ctx context.Context, database *db.DB, acct Account) (int, error) {
	...
	if err := database.MarkReactionCommandsSeeded(acct.AccountID); err != nil {
		return 0, err
	}
