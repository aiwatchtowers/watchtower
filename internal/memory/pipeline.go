...
// committed work stays) — EXCEPT a cancelled ctx mid-Reconcile (step 1),
// which is treated as an ordinary interrupted stop, not a failure: the run
// ends there and completes as 'done', the same shape step 3's per-window
// interruption gets below, never fatal()'s status='error'; a per-window AI
// failure in step 3 freezes the watermark for that window but never fails
// the run (window isolation, catchup-style) — it is recorded in the
// window's pipeline_steps row; a semantic step failure in step 4 is logged
// and skipped and never fails the run or moves a watermark. A disabled
// config is a full no-op: nothing written, no pipeline_runs row.
func (p *Pipeline) Run(ctx context.Context) (RunStats, error) {
	...
	stats.Reconciled, err = reconcile(ctx, p.vault, p.db, p.logf)
	if err != nil {
		if ctx.Err() != nil {
			// An ordinary daemon shutdown mid-reconcile, not a real
			// failure: reconcile() (internal/memory/index.go) already
			// stops between files on cancel and returns ctx.Err() itself
			// before its deletion loop and phase B, so everything indexed
			// so far is left fully consistent (see its own doc comment).
			// Mirrors extraction's identical "interrupted" shape below (it
			// breaks its batch loop on ctx.Err() and falls through to a
			// clean, nil-error completeRun) instead of the house "a
			// cancelled ctx is never recorded [as an error]" rule being
			// violated by routing through fatal(), which would record
			// status='error' for an ordinary stop.
			p.logf("memory: reconcile interrupted: %v", err)
			p.completeRun(runID, acc, stats.Episodes, wmBefore, wmBefore, nil)
			return stats, nil
		}
		return stats, p.fatal(runID, acc, &stats, wmBefore, err)
	}
	...
	// (5)-(6) Renders and the dark digest compare — see runDerived.
	p.runDerived(ctx, runID, semanticEnabled, acc, &stats)
...
// runFocusStep runs Run step 1b: focus salience when memory.focus.enabled, or
// its disable counterpart when the gate is off (a workspace that had focus on
// at some point must not keep a stale ×2.0/×0.5 skew forever — its own
// fingerprint-driven fast path keeps a never-enabled workspace byte-identical,
// final-review Fix 1). Source-isolated like calendar/mirrors/jira: a
// focus-step error is logged, never fatal, and never blocks the rest of the
// run. Returns the step count for numbering everything after it.
func (p *Pipeline) runFocusStep(runID int64, stats *RunStats) int {
...
// runDerived runs Run steps 5-6, all derived state that the next run
// re-renders: (5) the index.md/map.md renders (runRenders), then (6) the dark
// digest compare-mode (behind memory.renders.digest_compare), which renders each
// recently legacy-digested channel window from the memory episodes that now
// exist (extraction already ran this cycle) and shadow-stores the diff — a pure
// reader of digests/digest_topics/messages that writes only
...
// exist; source-isolated, never fatal. A cancelled run (daemon shutdown) skips
// both rather than writing after the stop — a strong map call would only fail
// there and fall back to a stub map.md commit.
func (p *Pipeline) runDerived(ctx context.Context, runID int64, semanticEnabled bool, acc *usageAccumulator, stats *RunStats) {
	if err := ctx.Err(); err != nil {
		p.logf("memory: renders skipped: %v", err)
		return
	}
...
// when memory is enabled) and map.md (the strong-tier hot summary when the
// semantic tier is on and within budget, else a mechanical fallback so MCP
// always has a map.md to read). Both are derived state — non-fatal, logged,
// and re-rendered next run.
func (p *Pipeline) runRenders(ctx context.Context, runID int64, semanticEnabled bool, acc *usageAccumulator) {
