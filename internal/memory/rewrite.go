...
//
// ctx is checked before each candidate: a cancelled run launches no further
// call, and a call killed by that cancellation is not stamped (cancelledCall).
// Pages already rewritten before the stop are still committed — their calls
// were paid for and stamped — and the step reports ctx.Err().
func (p *Pipeline) RewriteEntityPages(ctx context.Context, maxEntities int, now time.Time) (rewritten []string, failed int, usage *digest.Usage, err error) {
	...
	for _, row := range rows {
		...
		// A call killed by the run's own cancellation is not an attempt.
		if !cancelledCall(ctx, gerr) {
	...
	stopErr := ctx.Err() // reported once this run's paid-for work is stamped and committed
	if calls > 0 {
		usage = &acc
	...
	if len(nodes) == 0 {
		return rewritten, failed, usage, stopErr
	}
...
// cancelledCall reports whether a failed AI call died because the run's own ctx
// was cancelled (daemon shutdown) rather than failing on its own. Such a call
// never had a chance, so the rewrite and reflect memos do not stamp it as an
// attempt — stamping would cost the entity its weekly slot (the reflection its
// week) for a stop. The real generator reports a caller-side kill as its
// subprocess error ("signal: killed"), not as context.Canceled, so the test is
// the ctx itself, not the error chain.
func cancelledCall(ctx context.Context, err error) bool {
