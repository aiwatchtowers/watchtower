...
func TestAgeEpisodesTransitionsOldRawEpisodes(t *testing.T) {
	...
	aged, err := AgeEpisodes(context.Background(), v, d, 14, time.Now(), t.Logf)
	require.NoError(t, err)
...
// TestAgeEpisodes_CancelledCtxWritesNothing: a cancelled run returns before any
// write — no commit, the old episode is still active/short — and reports the
// cancellation; the next uncancelled run ages it as usual.
func TestAgeEpisodes_CancelledCtxWritesNothing(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	old := agingEpNode(t, "ep_01ARZ3NDEKTSV4RRFFQ69G5CA1", "C1CHAN", 30)
	writeAndIndex(t, v, d, old)
	head := headHash(t, v)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	aged, err := AgeEpisodes(ctx, v, d, 14, time.Now(), t.Logf)
	require.ErrorIs(t, err, context.Canceled)
	assert.Zero(t, aged)
	assert.Equal(t, head, headHash(t, v), "no commit lands after cancellation")
	got, err := v.ReadNode(old.ID)
	require.NoError(t, err)
	...
	aged, err = AgeEpisodes(context.Background(), v, d, 14, time.Now(), t.Logf)
	require.NoError(t, err)
	assert.Equal(t, 1, aged, "the next, uncancelled run ages it")
}
