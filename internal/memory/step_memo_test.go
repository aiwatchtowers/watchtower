...
// TestRewriteEntityPages_CancelledCallDoesNotStampAttempt: a strong-tier call
// killed by the run's own cancellation (daemon shutdown) is not an attempt —
// stamping it would cost the entity its weekly slot for a call that never had a
// chance. The real generator reports such a kill as its subprocess error
// ("signal: killed"), not as context.Canceled, so the fake does the same. After
// the cancel no further entity is called, and the next cycle of the same slot
// day re-attempts every entity.
func TestRewriteEntityPages_CancelledCallDoesNotStampAttempt(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	ids := seedRewriteFixture(t, v, d, 2)
	...
	rewritten, _, _, err := p.RewriteEntityPages(ctx, 10, rewriteNow)
	require.ErrorIs(t, err, context.Canceled, "the interrupted step reports the cancellation")
	assert.Empty(t, rewritten)
	require.Len(t, gen.calls, 1, "no further entity is called once the run is cancelled")

	stamps, err := d.MemoryStepStates(db.MemoryStepRewrite)
	require.NoError(t, err)
	assert.Empty(t, stamps, "a cancelled call stamps no attempt")

	gen.reply = memoRewriteGen(t).reply
	again, _, _, err := p.RewriteEntityPages(context.Background(), 10, rewriteNow.Add(70*time.Minute))
	require.NoError(t, err)
	assert.ElementsMatch(t, ids, again, "both entities keep their slot day")
}

// TestReflect_CancelledCallDoesNotStampAttempt: the weekly reflection memo is
// not stamped for a call killed by the run's own cancellation, so a shutdown on
// the slot day does not cost the week — the next cycle that day reflects.
func TestReflect_CancelledCallDoesNotStampAttempt(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	day := reflectDueDay("T1")
	gen, p, _ := reflectMemoFixture(t, v, d, day)
	okReply := gen.reply
	ctx, cancel := context.WithCancel(context.Background())
	gen.reply = func(string) (string, error) {
		cancel()
		return "", fmt.Errorf("signal: killed")
	}

	_, _, _, _, err := p.Reflect(ctx, day)
	require.Error(t, err)
	stamp, _, err := d.MemoryStepState(db.MemoryStepReflect, "")
	require.NoError(t, err)
	assert.Empty(t, stamp, "a cancelled call stamps no attempt")

	gen.reply = okReply
	n, _, _, _, err := p.Reflect(context.Background(), day.Add(70*time.Minute))
	require.NoError(t, err)
	assert.Equal(t, 1, n, "the slot day is not lost to the shutdown")
}

// TestReflect_CancelledCtxSkipsCall: a run already cancelled when reflection
// is reached makes no strong-tier call and stamps nothing.
func TestReflect_CancelledCtxSkipsCall(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	day := reflectDueDay("T1")
	gen, p, _ := reflectMemoFixture(t, v, d, day)
	ctx, cancel := context.WithCancel(context.Background())
	...
	assert.Empty(t, gen.calls, "no AI call on a cancelled run")
	stamp, _, err := d.MemoryStepState(db.MemoryStepReflect, "")
	require.NoError(t, err)
