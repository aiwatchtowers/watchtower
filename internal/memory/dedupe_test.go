...
func TestDedupeMergesSharedProvenance(t *testing.T) {
	...
	merged, err := DedupeEpisodes(context.Background(), v, d, 20, t.Logf)
...
func TestDedupeCapRespected(t *testing.T) {
	...
	merged, err := DedupeEpisodes(context.Background(), v, d, 1, t.Logf)
	require.NoError(t, err)
...
// TestDedupeEpisodes_CancelledCtxWritesNothing: a cancelled run stops before
// its first merge — no commit, both duplicates still live — and reports the
// cancellation; the next uncancelled run merges them.
func TestDedupeEpisodes_CancelledCtxWritesNothing(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	writeAndIndex(t, v, d, epNode("ep_01ARZ3NDEKTSV4RRFFQ69G5CD1", "First", "CDCHAN", "1752570000.000100"))
	writeAndIndex(t, v, d, epNode("ep_01ARZ3NDEKTSV4RRFFQ69G5CD2", "Second", "CDCHAN", "1752570000.000100"))
	head := headHash(t, v)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	merged, err := DedupeEpisodes(ctx, v, d, 20, t.Logf)
	...
	merged, err = DedupeEpisodes(context.Background(), v, d, 20, t.Logf)
	require.NoError(t, err)
	assert.Equal(t, 1, merged, "the next, uncancelled run merges the pair")
}
