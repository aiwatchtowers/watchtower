...
// OwnerEdited is the per-file form of the owner-touch signal: whether rel (a
// vault-relative slash path) was ever touched by a memory(owner-edit) commit,
// via go-git's FileName-filtered log. Test-only since 2026-09-25: that log walks
// the WHOLE history for any file never owner-edited (nearly all of them), and
// EvictEpisodes calling it once per candidate held a stopped daemon for ~3 h.
// Production reads the same fact from ownerEditedMemo (one walk per batch); the
// tests keep this form as the independent oracle the memo must agree with.
func (v *Vault) OwnerEdited(rel string) (bool, error) {
	v.ownerEditWalks.Add(1)
...
// WriteFile stages only the one file it writes: owner dirt sitting elsewhere
// in the worktree stays uncommitted, mirroring TestVaultWriteNodesStagingIsolation
// for WriteFile's own SkipStatus staging.
func TestVaultWriteFileStagingIsolation(t *testing.T) {
	dir := t.TempDir()
	v, err := OpenVault(dir)
	require.NoError(t, err)

	// Owner dirt: a modified tracked file (map.md, part of the init scaffold)
	// and a stray untracked file.
	require.NoError(t, os.WriteFile(filepath.Join(dir, "map.md"), []byte("# Owner edit\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "notes.md"), []byte("owner note\n"), 0o644))

	changed, err := v.WriteFile("index.md", []byte("# Memory Index\n\ncontent\n"), CommitMsg{Op: "index", Summary: "render index", Cause: "run:1"})
	require.NoError(t, err)
	require.True(t, changed)

	repo := openTestRepo(t, dir)
	assert.Equal(t, []string{"index.md"}, commitFiles(t, headCommit(t, repo)))
...
// TestOwnerEditedMemoMatchesPerFileWalk: the memo's one-walk owner-edited set
// answers exactly what the per-file walk (OwnerEdited) answers, path by path —
// the equivalence EvictEpisodes relies on since it moved onto the memo
// (MEM-03/MEM-07: an owner-edited episode keeps its eviction protection). The
// owner edits cover a modify, a rename (both its old and new path count, as in
// the per-file walk), a delete and an add, next to machine commits, an
// untouched file, and a path a LATER machine commit rewrites after the owner
// touched it (controller addendum, 2026-09-25): "ever edited by the owner" is
// a set membership, not "who touched it last" — a machine commit landing on
// top must not clear the protection.
func TestOwnerEditedMemoMatchesPerFileWalk(t *testing.T) {
	v := newTestVault(t)
	nodes := []Node{
		vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME1", "episode", "Modified"),
		vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME2", "episode", "Renamed"),
		vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME3", "episode", "Deleted"),
		vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME4", "episode", "Untouched"),
		vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME8", "episode", "OwnerThenMachine"),
	}
	writeNodes(t, v, nodes...)
	path := func(id string) string { return "episodes/" + id + ".md" }
	abs := func(rel string) string { return filepath.Join(v.path, filepath.FromSlash(rel)) }

	require.NoError(t, os.WriteFile(abs(path(nodes[0].ID)), append(nodes[0].Render(), []byte("\nhand edit\n")...), 0o644))
	renamedTo := path("ep_01ARZ3NDEKTSV4RRFFQ69G5ME5")
	require.NoError(t, os.Rename(abs(path(nodes[1].ID)), abs(renamedTo)))
	require.NoError(t, os.Remove(abs(path(nodes[2].ID))))
	added := path("ep_01ARZ3NDEKTSV4RRFFQ69G5ME6")
	require.NoError(t, os.WriteFile(abs(added), vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME6", "episode", "Added").Render(), 0o644))
	require.NoError(t, os.WriteFile(abs(path(nodes[4].ID)), append(nodes[4].Render(), []byte("\nhand edit\n")...), 0o644))
	made, err := v.CommitOwnerEdits()
	require.NoError(t, err)
	require.True(t, made)
	writeNodes(t, v, vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME7", "episode", "Machine"))

	// A machine commit rewrites the path the owner just touched — the owner's
	// earlier edit must still count.
	writeNodes(t, v, vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5ME8", "episode", "OwnerThenMachine rewritten"))

	mem := newOwnerEditedMemo(v)
	for _, rel := range []string{
		path(nodes[0].ID), path(nodes[1].ID), renamedTo, path(nodes[2].ID), added,
		path(nodes[3].ID), path("ep_01ARZ3NDEKTSV4RRFFQ69G5ME7"), path(nodes[4].ID),
	} {
		want, err := v.OwnerEdited(rel)
		require.NoError(t, err)
		got, err := mem.lookup(rel)
		require.NoError(t, err)
		assert.Equal(t, want, got, "memo and per-file walk disagree on %s", rel)
	}
	got, err := mem.lookup(renamedTo)
	require.NoError(t, err)
	assert.True(t, got, "sanity: the fixture exercises an owner-edited path")

	gotOwnerThenMachine, err := mem.lookup(path(nodes[4].ID))
	require.NoError(t, err)
	assert.True(t, gotOwnerThenMachine, "a later machine commit on the same path must not clear owner-edit protection")
	wantOwnerThenMachine, err := v.OwnerEdited(path(nodes[4].ID))
	require.NoError(t, err)
	assert.True(t, wantOwnerThenMachine, "sanity: the per-file oracle agrees")
}
