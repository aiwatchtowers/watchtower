...
// TestReconcile_CancelledCtxStopsEarly: a Reconcile whose run is cancelled
// mid-walk (here: right after the first file, a quarantined one) stops between
// files and reports the cancellation. It must not index the files it never
// reached, and — load-bearing — must not run the deletion loop, whose on-disk
// set is incomplete: a row whose file is merely unvisited would be deleted. A
// removed file's row therefore survives until the next complete pass, which
// converges the index as usual.
func TestReconcile_CancelledCtxStopsEarly(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	gone := vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5RC1", "episode", "Gone")
	writeNodes(t, v, gone)
	_, err := Reconcile(v, d, t.Logf)
	require.NoError(t, err)
	require.NoError(t, os.Remove(filepath.Join(v.path, "episodes", gone.ID+".md")))

	// entities/ is walked first: a malformed file there quarantines (and logs)
	// before episodes/ holds the fresh, never-indexed node.
	require.NoError(t, os.WriteFile(filepath.Join(v.path, "entities", "ent_01ARZ3NDEKTSV4RRFFQ69G5RC2.md"), []byte("not a node"), 0o644))
	fresh := vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5RC3", "episode", "Fresh")
	writeNodes(t, v, fresh)

	ctx, cancel := context.WithCancel(context.Background())
	cancelOnQuarantine := func(format string, args ...any) {
		t.Logf(format, args...)
		if strings.Contains(format, "quarantined") {
			cancel()
		}
	}
	stats, err := reconcile(ctx, v, d, cancelOnQuarantine)
	require.ErrorIs(t, err, context.Canceled)
	assert.Equal(t, 1, stats.Quarantined, "the file before the cancel was processed")
	assert.Zero(t, stats.Added, "no file after the cancel is indexed")
	assert.Zero(t, stats.Deleted, "the deletion loop never runs on a partial walk")
	_, err = d.GetMemoryNode(fresh.ID)
	require.ErrorIs(t, err, sql.ErrNoRows)
	_, err = d.GetMemoryNode(gone.ID)
	require.NoError(t, err, "the removed file's row survives the partial pass")

	stats, err = Reconcile(v, d, t.Logf)
	require.NoError(t, err)
	assert.Equal(t, 1, stats.Added)
	assert.Equal(t, 1, stats.Deleted, "the next complete pass converges")
}
