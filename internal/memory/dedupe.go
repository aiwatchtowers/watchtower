...
// candidate never stops the pass. ctx is checked before each merge — the only
// writes, each two commits (union, then Merge): a cancelled pass returns the
// merges already committed (each a complete unit) with ctx.Err(), never a
// half-made merge.
func DedupeEpisodes(ctx context.Context, v *Vault, database *db.DB, maxMerges int, logf func(string, ...any)) (merged int, err error) {
	...
	for _, eps := range byChannel {
		for i := range eps {
			...
			for j := i + 1; j < len(eps); j++ {
				...
				// Stop between merges, never inside one; the next run picks up
				// the rest.
