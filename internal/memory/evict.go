...
//
// The owner-touch signal for every candidate comes from ONE owner-edit
// history walk (ownerEditedMemo): the candidate scan is bounded by the vault,
// not by maxEvict (which caps evictions only), and a per-candidate history walk
// held a stopped daemon for hours (2026-09-25). ctx is checked before each
// candidate; everything is built in memory and written once after the loop, so
// a cancelled pass returns (0, ctx.Err()) having written nothing.
func EvictEpisodes(ctx context.Context, v *Vault, database *db.DB, olderThanDays int, scoreThreshold float64, maxEvict int, logf func(string, ...any)) (evicted int, err error) {
	...
	for _, row := range rows {
		...
		score, serr := candidateRetention(database, mem, n, ageDays)
		if serr != nil {
			return evicted, serr
		}
	...
	// The same memo serves the index mirror: this pass's own write is a machine
	// commit, which never changes the owner-edited set.
...
// candidateRetention scores one eviction candidate: its retention inputs read
// from the index (links-in, the net owner-engagement of the entities linking it
// — Phase-5 5D — and its focus state, 2026-07-26 owner verdict A: a live 'now'
// match resists eviction) plus the owner-touch signal from the pass's shared
// memo (MEM-03: an owner-edited episode keeps its bonus).
func candidateRetention(database *db.DB, mem *ownerEditedMemo, n Node, ageDays float64) (float64, error) {
	...
	ownerTouched, err := mem.lookup(rel)
	if err != nil {
		return 0, err
	}
	...
	return RetentionScore(RetentionInputs{
