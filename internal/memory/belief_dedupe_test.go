...
// Widened on 2026-09-23 (owner decision): shake joins the no-op gate
// alongside confirm and weaken. A shake citing only already-recorded evidence
// no longer flips an active belief to shaken from zero new information —
// mirrors TestReviseBeliefsWeakenWithAlreadyRecordedEvidenceIsNoOp. Only
// retire stays out (already evidence-gated in applyOp, never had the
// unconditional shape) — see docs/inventory/memory.md's changelog entry.
func TestReviseBeliefsShakeWithAlreadyRecordedEvidenceIsNoOp(t *testing.T) {
	tsFor := dedupeTS(5, "000100")
	tsAgainst := dedupeTS(5, "000200")
	bel := beliefTestNode("bel_00000000000000000000000001", "Alice is reliable", dedupeSubject, 0.5, 2, "active",
		beliefEvidence{Rank: rankObserved, Support: true, ChannelID: "C1CHAN", TS: tsFor},
	...
	touched, rejected := run(t, beliefOpJSON{BeliefID: bel.ID, Op: "shake",
		Evidence: []episodeRef{{ChannelID: "C1CHAN", TS: tsAgainst}}, Rationale: "still not sure"})

	got, err := v.ReadNode(bel.ID)
	require.NoError(t, err)
	assert.Equal(t, "active", got.Status, "a re-cited ref buys no status transition")
	assert.Equal(t, 0.5, got.Confidence, "shake never touches confidence")
	assert.Equal(t, 2, got.Stability, "shake never touches stability")
	assert.Equal(t, bel.Body, got.Body, "body byte-identical: no evidence line, no ## History line")
	assert.Zero(t, beliefCommitCount(t, v), "no vault commit — so reflection sees no churn")
	assert.Equal(t, 0, touched, "the op reaches applyOp for neither branch...")
	assert.Equal(t, 0, rejected, "...so RunStats.BeliefOpsRejected stays honest: this was never refused by the rank math")
...
// The no-op rule is ALL cited refs already stored, never ANY: a shake citing
// one new ref still applies in full, flipping active -> shaken — mirrors
// TestReviseBeliefsWeakenWithOneNewRefStillApplies.
func TestReviseBeliefsShakeWithOneNewRefStillApplies(t *testing.T) {
	tsFor := dedupeTS(5, "000100")
	tsAgainst := dedupeTS(5, "000200")
	tsFresh := dedupeTS(4, "000300")
	bel := beliefTestNode("bel_00000000000000000000000001", "Alice is reliable", dedupeSubject, 0.5, 2, "active",
	...
	touched, rejected := run(t, beliefOpJSON{BeliefID: bel.ID, Op: "shake", Evidence: []episodeRef{
		{ChannelID: "C1CHAN", TS: tsAgainst}, // already recorded
		{ChannelID: "C1CHAN", TS: tsFresh},   // new
	}, Rationale: "one old, one new"})
	...
	got, err := v.ReadNode(bel.ID)
	require.NoError(t, err)
	assert.Equal(t, "shaken", got.Status, "partly-new evidence still shakes the belief")
	assert.Equal(t, 0.5, got.Confidence, "shake never touches confidence")
	assert.Equal(t, 1, countEvidenceRef(got.Body, "C1CHAN "+tsAgainst), "the stored ref is not duplicated")
	assert.Equal(t, 1, countEvidenceRef(got.Body, "C1CHAN "+tsFresh), "the new ref is appended once")
	assert.Equal(t, 1, beliefCommitCount(t, v))
