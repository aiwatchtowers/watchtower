...
func TestEvictEngagedEpisodeSurvivesTwinEvicts(t *testing.T) {
	...
	evicted, err := EvictEpisodes(context.Background(), v, d, 45, 0.5, 20, t.Logf)
...
func TestMemory07_EvictionKeepsProvenance(t *testing.T) {
	...
	merged, err := DedupeEpisodes(context.Background(), v2, d2, 20, t.Logf)
	require.NoError(t, err)
...
func TestEvictAppendsToExistingRollup(t *testing.T) {
	...
	n1, err := EvictEpisodes(context.Background(), v, d, 45, 0.5, 20, t.Logf)
	require.NoError(t, err)
	require.Equal(t, 1, n1)

	e2 := oldEpNode(t, "ep_01ARZ3NDEKTSV4RRFFQ69G5EV7", "Second cold", "closed", "C4CHAN", 100)
	writeAndIndex(t, v, d, e2)
	n2, err := EvictEpisodes(context.Background(), v, d, 45, 0.5, 20, t.Logf)
	require.NoError(t, err)
...
func TestEvictCapRespected(t *testing.T) {
	...
	evicted, err := EvictEpisodes(context.Background(), v, d, 45, 0.5, 1, t.Logf)
	require.NoError(t, err)
...
// TestEvictEpisodes_OwnerTouchReadsHistoryOnce pins the 2026-09-25 shutdown-
// hang fix: the owner-touch signal for every eviction candidate comes from ONE
// full-history walk (the ownerEditedMemo), not one walk per candidate — a
// per-candidate walk re-diffed the whole commit history for each candidate and
// held a stopped daemon for ~3 h. Several candidates, one of them owner-edited:
// the history is walked exactly once for the whole pass (including the index
// mirror after the write), and the owner-edited candidate is still protected
// (MEM-03/MEM-07) while its cold siblings are evicted.
func TestEvictEpisodes_OwnerTouchReadsHistoryOnce(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	var cold []Node
	for i := 0; i < 3; i++ {
		ep := oldEpNode(t, fmt.Sprintf("ep_01ARZ3NDEKTSV4RRFFQ69G5HO%d", i), "Cold", "done", fmt.Sprintf("C8CHA%d", i), 100)
		writeAndIndex(t, v, d, ep)
		cold = append(cold, ep)
	}
	owned := oldEpNode(t, "ep_01ARZ3NDEKTSV4RRFFQ69G5HO9", "Owner note", "done", "C8CHAN", 100)
	writeAndIndex(t, v, d, owned)
	rel, err := nodeRelPath(owned.ID)
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(v.path, filepath.FromSlash(rel)), append(owned.Render(), []byte("\nOwner annotation.\n")...), 0o644))
	committed, err := v.CommitOwnerEdits()
	require.NoError(t, err)
	require.True(t, committed)

	before := v.ownerEditWalks.Load()
	evicted, err := EvictEpisodes(context.Background(), v, d, 45, 0.5, 20, t.Logf)
	require.NoError(t, err)

	assert.Equal(t, int64(1), v.ownerEditWalks.Load()-before, "one history walk for all %d candidates", len(cold)+1)
	assert.Equal(t, len(cold), evicted, "every cold, untouched candidate is evicted")
	kept, err := v.ReadNode(owned.ID)
	require.NoError(t, err)
	assert.Equal(t, "closed", kept.Status, "the owner-edited candidate keeps its owner-touch protection")
}

// TestEvictEpisodes_CancelledCtxWritesNothing: a cancelled run stops before the
// candidate scan's first git read and before any write — no commit, no
// tombstone, and the cancellation is reported (a shutdown must not be held by
// the scan, and must not leave a half-evicted vault).
func TestEvictEpisodes_CancelledCtxWritesNothing(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	ep := oldEpNode(t, "ep_01ARZ3NDEKTSV4RRFFQ69G5CX1", "Cold retro", "done", "C9CHAN", 100)
	writeAndIndex(t, v, d, ep)
	head := headHash(t, v)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	evicted, err := EvictEpisodes(ctx, v, d, 45, 0.5, 20, t.Logf)
	require.ErrorIs(t, err, context.Canceled)
	assert.Zero(t, evicted)
	assert.Equal(t, head, headHash(t, v), "no commit lands after cancellation")
	got, err := v.ReadNode(ep.ID)
	require.NoError(t, err)
	assert.Equal(t, "closed", got.Status, "the candidate is not tombstoned")

	evicted, err = EvictEpisodes(context.Background(), v, d, 45, 0.5, 20, t.Logf)
	require.NoError(t, err)
	assert.Equal(t, 1, evicted, "the next, uncancelled run evicts it")
}
