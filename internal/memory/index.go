...
func Reconcile(v *Vault, database *db.DB, logf func(string, ...any)) (Stats, error) {
	return reconcile(context.Background(), v, database, logf)
}

// reconcile is Reconcile under a run's ctx — the memory pipeline's entry, so a
// daemon stop is honoured between files (a Reconcile ran 7.5 h on a bloated
// vault on 2026-09-25, ctx-blind). A cancelled pass returns ctx.Err() before
// the deletion loop and phase B: every file visited so far is indexed exactly
// as a complete pass would index it (each upsert is its own consistent row),
// unvisited files keep their previous rows, and no row is deleted — the
// on-disk set is incomplete, so the deletion loop would drop live rows. The
// next complete pass converges the rows. One residual: phase B's refinement
// of the files this pass touched is lost (their content hash now matches, so
// the next pass skips them), leaving their importance_score at the phase-A
// value until the node or a node linking it changes, or `memory reindex`.
func reconcile(ctx context.Context, v *Vault, database *db.DB, logf func(string, ...any)) (Stats, error) {
	if logf == nil {
