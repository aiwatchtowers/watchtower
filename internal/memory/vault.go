...
type Vault struct {
	...
	// ownerEditWalks counts full commit-history walks for the owner-edit
	// signal — a test seam pinning that a batch pays one walk, not one per
	// node (the 2026-09-25 eviction shutdown hang).
	ownerEditWalks atomic.Int64
}
...
// memory(owner-edit) commit, across the FULL history, in ONE walk — the
// owner-touch input to the retention and importance scores, for every file at
// once. It reads every commit header but diffs only the owner-edit commits,
// each against its first parent without rename detection, and records both
// sides of every change: exactly the per-path test go-git's FileName-filtered
// log applies, so a rename marks its old and its new path alike. That per-file
// form cost one whole-history walk per file (nearly every file was never owner-
// edited, so the walk never stopped early) — EvictEpisodes calling it once per
// candidate held a stopped daemon for ~3 h on 2026-09-25; every caller now
// reads this set through ownerEditedMemo. The vault history is linear (single
// author, no merges — same invariant LogMemoryCommits relies on), so the
// first-parent diff is exact, not an approximation.
func (v *Vault) OwnerEditedFiles() (map[string]bool, error) {
	...
	err = iter.ForEach(func(c *object.Commit) error {
		...
		return addChangedPaths(c, files)
	})
	if err != nil {
...
// addChangedPaths adds every path c changed against its first parent (the
// empty tree for a root commit) to into — both sides of each change, with no
// rename detection.
func addChangedPaths(c *object.Commit, into map[string]bool) error {
	tree, err := c.Tree()
	if err != nil {
		return err
	}
...
// (sibling of OwnerEditedFiles). The vault history is linear (single author, no
...
func (v *Vault) WriteNodes(nodes []Node, msg CommitMsg) (string, error) {
	...
	for _, n := range nodes {
		...
		// SkipStatus: this stages exactly rel — doAdd skips the whole-worktree
		// Status() walk it would otherwise run first (worktree_status.go's
		// doAdd/doAddFile: with skipStatus and a file path, s stays nil, so
		// doAddFile's "s != nil && Unmodified" short-circuit never fires and it
		// hashes+indexes only path). No other worktree dirt is touched (MEM-03).
...
func (v *Vault) WriteFile(rel string, content []byte, msg CommitMsg) (bool, error) {
	...
	// SkipStatus: see the matching comment in WriteNodes.
	if err := wt.AddWithOptions(&git.AddOptions{Path: rel, SkipStatus: true}); err != nil {
