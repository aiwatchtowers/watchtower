...
// message ref ("1:C0FAKE63") must dispatch to the message resolver (scheme
...
func TestProvenanceRegistryDispatchesNamespacedSlackMessage(t *testing.T) {
	d := newTestDB(t)
	_, err := d.Exec(`INSERT INTO messages (channel_id, ts, user_id, text) VALUES ('1:C0FAKE63', '100.000100', '1:U1', 'hi')`)
	require.NoError(t, err)
	reg := fullRegistry(d)

	ok, registered, err := reg.Validate(episodeRef{ChannelID: "1:C0FAKE63", TS: "100.000100"})
	require.NoError(t, err)
