...
// commitSubjectsSince returns the subject line of every vault commit made after
// from (a commit hash), newest first.
func commitSubjectsSince(t *testing.T, v *Vault, from string) []string {
	t.Helper()
	...
	for {
		c, err := iter.Next()
		require.NoError(t, err, "walked past the starting commit")
...
// TestPipeline_CancelledCtxSkipsPostBeliefSteps pins the 2026-09-25 shutdown-
// hang fix at the pipeline level: once the run's ctx is cancelled (here by the
// first strong-tier rewrite call, the way a daemon stop lands mid-call), no
// later step starts — no belief pass, no aging, no eviction, no index/map
// render — so nothing is written after the stop, and every step that never ran
// records a 'skipped' row instead of the run ending silently. Each of those
// steps has due work in the fixture, so a step that ran would leave a commit.
func TestPipeline_CancelledCtxSkipsPostBeliefSteps(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	pipelineFixture(t, d)
	now := time.Now()
	agable := rewriteEpisodeNode("ep_00000000000000000000000001", "C1CHAN", "1710000000.000100") // years old, active/short
	writeAndIndex(t, v, d, agable)
	writeAndIndex(t, v, d, rewriteEntityNode(dueEntityIDs(now, 1)[0], "Ent", agable.ID))
	writeAndIndex(t, v, d, oldEpNode(t, "ep_01ARZ3NDEKTSV4RRFFQ69G5PC1", "Cold retro", "done", "C9CHAN", 100))
	head := headHash(t, v)

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	gen := &fakeGen{reply: func(user string) (string, error) {
		if strings.Contains(user, "Entity page:") {
			cancel()
	...
	_, err := p.Run(ctx)
	require.NoError(t, err, "a cancelled semantic tier never fails the run")

	var rewriteCalls int
	for _, c := range gen.calls {
		rewriteCalls += strings.Count(c, "Entity page:")
		assert.NotContains(t, c, "Existing beliefs:", "no belief pass after the cancel")
		assert.NotContains(t, c, "Top entities:", "no strong map render after the cancel")
	}
	require.Equal(t, 1, rewriteCalls, "the fixture must reach the rewrite call that cancels")

	for _, s := range commitSubjectsSince(t, v, head) {
		for _, op := range []string{"beliefs", "age", "evict", "reflect", "index", "map"} {
			assert.False(t, strings.HasPrefix(s, "memory("+op+")"), "no %s commit after the cancel, got %q", op, s)
		}
	}
	...
	assert.Equal(t, "error", byName["rewrite"], "the interrupted step reports the cancellation")
	for _, name := range []string{"beliefs", "age", "evict", "reflect"} {
		assert.Equal(t, "skipped", byName[name], "%s records a skipped row", name)
	}
}

// TestPipeline_ReconcileInterruptedEndsRunCleanly pins the fix for an
// ordinary daemon shutdown landing mid-Reconcile (Run step 1): before this
// fix, ANY reconcile error — including ctx.Err() from a plain stop —
// routed through fatal() and recorded pipeline_runs.status='error', the
// same shape a real reconcile failure gets. That violates the house rule
// "a cancelled ctx is never recorded [as an error]" that every OTHER step
// in this pipeline already follows (see TestPipeline_CancelledCtxSkipsPostBeliefSteps
// above, and extraction's identical "interrupted, N windows left" shape).
// An already-cancelled ctx makes reconcile()'s per-entry ctx.Err() check
// (internal/memory/index.go) fire on the very first vault entry, so this
// exercises the exact codepath without needing a mid-walk trigger.
func TestPipeline_ReconcileInterruptedEndsRunCleanly(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	pipelineFixture(t, d)
	// At least one node file so reconcile's subdirectory walk has an entry
	// to check ctx against — an empty vault never reaches the ctx.Err()
	// check at all (nothing to range over), which would trivially pass
	// through the whole loop uncancelled.
	writeNodes(t, v, vaultTestNode("ep_01ARZ3NDEKTSV4RRFFQ69G5RC1", "episode", "Something"))

	gen := &fakeGen{reply: func(string) (string, error) {
		t.Fatal("a reconcile interrupted before it even starts must stop the run before any extraction AI call")
		return "", nil
	}}
	cfg := pipelineTestConfig()
	p := NewPipeline(d, v, gen, cfg, t.Logf)
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel() // already cancelled before Run reaches reconcile

	stats, err := p.Run(ctx)
	require.NoError(t, err, "an ordinary shutdown mid-reconcile must not fail the run")
	assert.Zero(t, stats.Reconciled.Added, "reconcile stopped before indexing the seeded node")
	assert.Empty(t, gen.calls, "no AI call once the run ends this early")

	_, status, _, _, _, _, _, _, _ := memoryPipelineRunRow(t, d)
	assert.Equal(t, "done", status, "a cancelled-mid-reconcile run must complete as 'done', not 'error' — an ordinary daemon stop is not a failure")
}

// TestPipeline_ReconcileGenuineErrorStillRecordsRunAsError is the negative
// pair of TestPipeline_ReconcileInterruptedEndsRunCleanly above: the new
// ctx.Err() != nil branch in Run's reconcile handling must be scoped to an
// actual cancellation, not "any reconcile error skips fatal()". Without this
// test, deleting that condition entirely (always treating a reconcile error
// as non-fatal) would leave the whole suite green. Forces a genuine,
// non-cancellation reconcile error via the smallest available seam: replace
// the "entities" subdirectory (walked first, vaultSubdirs order) with a
// regular file, so os.ReadDir fails with a real IO error unrelated to ctx.
func TestPipeline_ReconcileGenuineErrorStillRecordsRunAsError(t *testing.T) {
	v, d := newTestVault(t), newTestDB(t)
	pipelineFixture(t, d)

	entitiesDir := filepath.Join(v.path, "entities")
	require.NoError(t, os.RemoveAll(entitiesDir))
	require.NoError(t, os.WriteFile(entitiesDir, []byte("not a directory"), 0o644))

	gen := &fakeGen{reply: func(string) (string, error) {
		t.Fatal("a failed reconcile must stop the run before any extraction AI call")
		return "", nil
	}}
	cfg := pipelineTestConfig()
	p := NewPipeline(d, v, gen, cfg, t.Logf)
	...
	_, err := p.Run(context.Background())
	require.Error(t, err, "a genuine (non-cancellation) reconcile error must still fail the run")
