...
// which does two things. (1) A `confirm`, `weaken`, or `shake` that cites ONLY
// evidence already on the page is a no-op: without this, the same ref re-cited
// on every daemon cycle would bump stability and confidence (confirm) —
// pushing flipThreshold(stability) toward practical un-retirability — drain
// confidence (weaken), or flip an active belief to shaken (shake), each time
// on evidence the belief never earned; all three produce commits reflection
// reads back as "flapping". The no-op rule is ALL kept refs already
// stored, never any: an op carrying one new ref is genuinely new evidence and
// applies in full. `shake` joined the gate on 2026-09-23 (owner decision) — it
// carries a real active->shaken status transition, so widening it to a no-op
// is an owner-visible behavior change, not a mechanical mirror of the
// confirm/weaken fix, and was deliberately left out until that call was made.
// `retire` stays out: its decision is already evidence-gated in applyOp
// (hasFreshOwnerSupport/flipSupported read the combined evidence), so it never
// had the unconditional-step shape the other three had. (2) For EVERY op, a
...
// Both changes move the math strictly more conservative, never less.
func (p *Pipeline) applyExistingOp(op beliefOpJSON, candidatesByID map[string]Node, kept []episodeRef, now time.Time) (node Node, applied, mathRejected bool) {
	...
	if (thisOp == opConfirm || thisOp == opWeaken || thisOp == opShake) && len(newEv) == 0 {
		// Not mathRejected: RunStats.BeliefOpsRejected means "refused by the rank
		// math", and this op never reached it. Scoped to confirm+weaken+shake per
		// owner decision 2026-09-23 — retire's decision is already evidence-gated,
		// so it is never widened here.
