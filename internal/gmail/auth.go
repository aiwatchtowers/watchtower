...
// injected by the caller (see cmd) — the calendar package's Default* values
// are reused there; this package intentionally has no defaults of its own.
...
func Prepare(cfg GoogleOAuthConfig, customRedirectURI string) (*PrepareResult, error) {
	...
	authorizeURL := buildAuthURL(cfg, redirectURI, state, pkce.Challenge)
...
func buildAuthURL(cfg GoogleOAuthConfig, redirectURI, state, codeChallenge string) string {
	params := url.Values{
...
func Login(ctx context.Context, cfg GoogleOAuthConfig, out io.Writer, opts ...LoginOptions) (*OAuthToken, error) {
	...
	if opt.SkipBrowserOpen {
		...
		fmt.Fprintf(out, "Opening browser for Gmail authorization...\n")
...
func listenLocal() (net.Listener, error) {
	preferred := []int{18521, 18522, 18523, 18524, 18525, 18526, 18527, 18528, 18529, 18530}
