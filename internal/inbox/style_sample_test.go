...
func TestStyleSample_NoWorkspaceErrorsCleanly(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "style sample needs a connected Slack account")
	assert.Equal(t, 0, gen.calls)
}

// A Google-only owner is a known owner but has no Slack messages to sample:
// the same clean error, no AI call.
func TestStyleSample_GoogleOnlyOwnerNeedsSlack(t *testing.T) {
	d := newTestDB(t)
	_, err := d.CreateGoogleAccount(db.GoogleAccount{Email: "me@x.com"})
	require.NoError(t, err)
