...
func (p *Pipeline) Run(ctx context.Context) (int, int, error) {
	...
	if !owner.Known() {
		p.logger.Println("tracks: no owner identity, skipping")
		return 0, 0, nil
	}
	...
	stored, err := p.RunForWindow(ctx, owner, from, to, digestsSinceISO)
	return stored, 0, err
}
...
// resets the cross-channel cache so it rebuilds lazily for this run. The
// profile goes through GetOwnerProfile, so a profile still parked under an
// earlier owner key is found (OWNER-01).
func (p *Pipeline) loadWindowContext(owner db.Owner) (*db.UserProfile, []db.Track) {
	profile, err := p.db.GetOwnerProfile(owner)
	if err != nil {
...
func scoreChannel(channelID string, topics []db.DigestTopic, userID string,
	...
	// userID (the owner's id, db.ResolveOwner) is namespaced ("1:U123"), but topic text is
