...
func TestScoreChannel(t *testing.T) {
	...
	// userID (the owner's id, db.ResolveOwner) is namespaced ("1:U1"), while the
...
// OWNER-01: the window context loads the owner's profile through
// GetOwnerProfile, so a profile still parked under another key (the Desktop's
// "pending:owner" row from a no-owner onboarding) is used for an owner whose
// ID is different.
func TestLoadWindowContext_UsesProfileParkedUnderAnotherKey(t *testing.T) {
	database := testDB(t)
	require.NoError(t, database.UpsertUserProfile(db.UserProfile{
		SlackUserID: "pending:owner",
		Role:        "middle_management",
	}))
	pipe := New(database, testConfig(), nil, log.Default())

	profile, _ := pipe.loadWindowContext(db.Owner{ID: "google:me@x.com", Source: db.OwnerSourceGoogle})
