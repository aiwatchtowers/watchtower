...
func TestPipelineRunNoUser(t *testing.T) {
	...
	id, err := pipe.Run(context.Background())
	// No owner is ErrNoOwner, not the (0, nil) "nothing to summarize" shape.
	require.ErrorIs(t, err, db.ErrNoOwner)
	assert.Equal(t, 0, id)
	assert.Equal(t, 0, gen.calls, "no owner never reaches the model")
}
