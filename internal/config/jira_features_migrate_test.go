...
// an EXPLICIT value for every toggle — the common shape being all-false — freezing the
...
func TestMigrateJiraFeatureKeys_CarriesEnablesAndDropsArtifactFalses(t *testing.T) {
	...
	// The retired who_ping / write_back_suggestions toggles are not in the
	// table, so their squashed keys are left in place — inert, as
	// TestLoad_RetiredJiraFeatureKeysAreInert pins.
	for _, retired := range []string{"whoping", "writebacksuggestions"} {
		assert.True(t, v.IsSet("jira.features."+retired), "the retired key %q is left untouched", retired)
	}
