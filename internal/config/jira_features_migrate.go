...
// JiraFeatureToggles by reflection, so a renamed field cannot drift. The
// retired who_ping/write_back_suggestions toggles (whoping,
// writebacksuggestions) are deliberately absent: a squashed key of theirs on
// an unrepaired install is left in place, where — like their readable
// spellings — it decodes into nothing and is inert.
...
// write an explicit value for every toggle onto every repaired install — the common
