...
// userRole returns the owner's configured role from their profile, or "".
// userID is the owner id the plan is keyed by (RunOptions.UserID).
func (p *Pipeline) userRole(userID string) string {
	if prof, err := p.db.GetOwnerProfile(db.Owner{ID: userID}); err == nil && prof != nil {
		return prof.Role
	}
	return ""
}
