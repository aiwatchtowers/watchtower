...
func (p *Pipeline) loadCaches() {
	...
	// An unknown owner has no profile: GetOwnerProfile returns nil for it.
	if owner, err := p.db.ResolveOwner(); err != nil {
		p.logger.Printf("warning: resolving owner for profile cache: %v", err)
	} else if profile, err := p.db.GetOwnerProfile(owner); err == nil {
		p.profile = profile
	}
